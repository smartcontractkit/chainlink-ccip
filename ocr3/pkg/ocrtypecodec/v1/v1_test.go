...
func TestExecObservation(t *testing.T) {
	...
	for _, gen := range dataGenerators {
		result := runBenchmark(
			...
			gen.execObservation(),
			func(b []byte) (interface{}, error) { return jsonCodec.DecodeObservation(b) },
			func(i interface{}) ([]byte, error) { return jsonCodec.EncodeObservation(i.(exectypes2.Observation)) },
			func(b []byte) (interface{}, error) { return protoCodec.DecodeObservation(b) },
			func(i interface{}) ([]byte, error) { return protoCodec.EncodeObservation(i.(exectypes2.Observation)) },
		)
		results = append(results, result)
	}
	fmt.Println(resultDataArray(results))
...
func TestExecOutcome(t *testing.T) {
	...
	for _, gen := range dataGenerators {
		result := runBenchmark(
			...
			gen.execOutcome(),
			func(b []byte) (interface{}, error) { return jsonCodec.DecodeOutcome(b) },
			func(i interface{}) ([]byte, error) { return jsonCodec.EncodeOutcome(i.(exectypes2.Outcome)) },
			func(b []byte) (interface{}, error) { return protoCodec.DecodeOutcome(b) },
			func(i interface{}) ([]byte, error) { return protoCodec.EncodeOutcome(i.(exectypes2.Outcome)) },
		)
		results = append(results, result)
	}
	fmt.Println(resultDataArray(results))
...
func (d *dataGenerator) execObservation() exectypes2.Observation {
	discoveryObs := genDiscoveryObservation(d.numSourceChains, d.numContractsPerChain)

	msgs := genMessages(d.numMessagesPerChain, d.numTokensPerMsg)

	tokenDataObservations := make(map[cciptypes.ChainSelector]map[cciptypes.SeqNum]exectypes2.MessageTokenData)
	...
	for i := 0; i < d.numSourceChains; i++ {
		...
		tokenDataObservations[chainSel] = make(map[cciptypes.SeqNum]exectypes2.MessageTokenData, d.numMessagesPerChain)
		...
		for j := 0; j < d.numMessagesPerChain; j++ {
			tokenDataObservations[chainSel][cciptypes.SeqNum(rand.Uint64())] = exectypes2.MessageTokenData{
				TokenData: []exectypes2.TokenData{
					{
						Ready:     rand.Int()%2 == 0,
	...
	commitReports := make(map[cciptypes.ChainSelector][]exectypes2.CommitData)
	for i := 0; i < d.numSourceChains; i++ {
		commitReports[cciptypes.ChainSelector(rand.Uint64())] = []exectypes2.CommitData{
			{
				...
				MessageTokenData: []exectypes2.MessageTokenData{
					{
						TokenData: []exectypes2.TokenData{
							{
								Ready:     rand.Int()%2 == 0,
...
func (d *dataGenerator) execOutcome() exectypes2.Outcome {
	commitReports := make([]exectypes2.CommitData, d.numSourceChains)
	msgs := genMessages(d.numMessagesPerChain, d.numTokensPerMsg)

	for i := 0; i < d.numSourceChains; i++ {
		commitReports[i] = exectypes2.CommitData{
			SourceChain:   cciptypes.ChainSelector(rand.Uint64()),
			OnRampAddress: randomBytes(40),
			Timestamp:     time.Now().UTC(),
			BlockNum:      rand.Uint64(),
	...
	return exectypes2.Outcome{
		State:         exectypes2.PluginState(genRandomString(128)),
		CommitReports: commitReports,

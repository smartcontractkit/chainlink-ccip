...
import (
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/commit/chainfee"
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/pkg/ocrtypecodec/v1/ocrtypecodecpb"
...
func (t *protoTranslator) commitDataSliceToProto(commits []exectypes2.CommitData) []*ocrtypecodecpb.CommitData {
...
func (t *protoTranslator) commitReportsFromProto(
	...
	var commitReports exectypes2.CommitObservations
	if len(pbObservations) > 0 {
		commitReports = make(exectypes2.CommitObservations, len(pbObservations))
	}
...
func (t *protoTranslator) commitDataSliceFromProto(pbCommits []*ocrtypecodecpb.CommitData) []exectypes2.CommitData {
	commitData := make([]exectypes2.CommitData, len(pbCommits))

	for i, commit := range pbCommits {
		commitData[i] = exectypes2.CommitData{
...
func (t *protoTranslator) messageTokenDataSliceToProto(
	data []exectypes2.MessageTokenData,
...
func (t *protoTranslator) messageTokenDataToProto(data exectypes2.MessageTokenData) *ocrtypecodecpb.MessageTokenData {
...
func (t *protoTranslator) messageObservationsToProto(
	msgs exectypes2.MessageObservations,
...
func (t *protoTranslator) messageObservationsFromProto(
	...
	var messages exectypes2.MessageObservations
	if len(pbMsgs) > 0 {
		messages = make(exectypes2.MessageObservations, len(pbMsgs))
	}
...
func (t *protoTranslator) messageHashesFromProto(
	...
	var hashes exectypes2.MessageHashes
	if len(pbHashes) > 0 {
		hashes = make(exectypes2.MessageHashes, len(pbHashes))
	}
...
func (t *protoTranslator) tokenDataObservationsToProto(
	observations exectypes2.TokenDataObservations,
...
func (t *protoTranslator) tokenDataObservationsFromProto(
	...
	var tokenDataObservations exectypes2.TokenDataObservations
	if len(pbObservations) > 0 {
		tokenDataObservations = make(exectypes2.TokenDataObservations, len(pbObservations))
	}

	for chainSel, tokenMap := range pbObservations {
		innerMap := make(map[cciptypes.SeqNum]exectypes2.MessageTokenData, len(tokenMap.TokenData))
...
func (t *protoTranslator) nonceObservationsToProto(
	observations exectypes2.NonceObservations,
...
func (t *protoTranslator) nonceObservationsFromProto(
	...
	var nonces exectypes2.NonceObservations
	if len(pbObservations) > 0 {
		nonces = make(exectypes2.NonceObservations, len(pbObservations))
	}
...
func (t *protoTranslator) decodeMessageTokenData(data []*ocrtypecodecpb.MessageTokenData) []exectypes2.MessageTokenData {
	var result []exectypes2.MessageTokenData
	if len(data) > 0 {
		result = make([]exectypes2.MessageTokenData, len(data))
	}
...
) exectypes2.MessageTokenData {
	tokenData := make([]exectypes2.TokenData, len(data.TokenData))
	for i, td := range data.TokenData {
		tokenData[i] = exectypes2.TokenData{
			Ready: td.Ready,
			Data:  td.Data,
		}
	}
	return exectypes2.MessageTokenData{TokenData: tokenData}
}

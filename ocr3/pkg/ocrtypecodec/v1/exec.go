...
import (
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/internal/plugincommon/discovery/discoverytypes"
...
type ExecCodec interface {
	EncodeObservation(observation exectypes2.Observation) ([]byte, error)
	DecodeObservation(data []byte) (exectypes2.Observation, error)

	EncodeOutcome(outcome exectypes2.Outcome) ([]byte, error)
	DecodeOutcome(data []byte) (exectypes2.Outcome, error)
}
...
func (e *ExecCodecProto) EncodeObservation(observation exectypes2.Observation) ([]byte, error) {
...
func (e *ExecCodecProto) DecodeObservation(data []byte) (exectypes2.Observation, error) {
	if len(data) == 0 {
		return exectypes2.Observation{}, nil
	}

	pbObs := &ocrtypecodecpb.ExecObservation{}
	if err := proto.Unmarshal(data, pbObs); err != nil {
		return exectypes2.Observation{}, fmt.Errorf("proto unmarshal ExecObservation: %w", err)
	}
...
func (e *ExecCodecProto) EncodeOutcome(outcome exectypes2.Outcome) ([]byte, error) {
...
func (e *ExecCodecProto) DecodeOutcome(data []byte) (exectypes2.Outcome, error) {
	if len(data) == 0 {
		return exectypes2.Outcome{}, nil
	}

	pbOtcm := &ocrtypecodecpb.ExecOutcome{}
	if err := proto.Unmarshal(data, pbOtcm); err != nil {
		return exectypes2.Outcome{}, fmt.Errorf("proto unmarshal ExecOutcome: %w", err)
	}

	otcm := exectypes2.Outcome{
		State:         exectypes2.PluginState(pbOtcm.PluginState),
...
func (*ExecCodecJSON) EncodeObservation(observation exectypes2.Observation) ([]byte, error) {
	return json.Marshal(observation)
}

func (*ExecCodecJSON) DecodeObservation(data []byte) (exectypes2.Observation, error) {
	if len(data) == 0 {
		return exectypes2.Observation{}, nil
	}
...
func (*ExecCodecJSON) EncodeOutcome(outcome exectypes2.Outcome) ([]byte, error) {
...
func (*ExecCodecJSON) DecodeOutcome(data []byte) (exectypes2.Outcome, error) {
	if len(data) == 0 {
		return exectypes2.Outcome{}, nil
	}
	o := exectypes2.Outcome{}

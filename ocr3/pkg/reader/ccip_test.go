...
import (
	...
	commonccipocr3 "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/chainlink_common/ccipocr3"
	writermocks "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/chainlink_common/types"
	contractreader3 "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/pkg/contractreader"
	"github.com/smartcontractkit/chainlink-ccip/ocr3/pkg/addressbook"
	"github.com/smartcontractkit/chainlink-ccip/ocr3/pkg/consts"
	contractreader2 "github.com/smartcontractkit/chainlink-ccip/ocr3/pkg/contractreader"
)
...
func TestCCIPChainReader_Sync_HappyPath_BindsContractsSuccessfully(t *testing.T) {
	...
	destExtended := contractreader3.NewMockExtended(t)
	source1Extended := contractreader3.NewMockExtended(t)
	source2Extended := contractreader3.NewMockExtended(t)
	...
	ccipReader, err := newCCIPChainReaderInternal(
		...
		logger.Test(t),
		chainAccessors,
		map[cciptypes.ChainSelector]contractreader2.ContractReaderFacade{
...
func TestCCIPChainReader_DiscoverContracts_HappyPath_Round1(t *testing.T) {
	...
	mockReaders := make(map[cciptypes.ChainSelector]*contractreader3.MockExtended)
	mockReaders[destChain] = contractreader3.NewMockExtended(t)
	...
	mockCache.On("GetChainConfig", mock.Anything, sourceChain[0]).Return(
		cciptypes.ChainConfigSnapshot{}, contractreader2.ErrNoBindings).Maybe()
	...
	castToExtended := make(map[cciptypes.ChainSelector]contractreader2.Extended)
...
func TestCCIPChainReader_DiscoverContracts_HappyPath_Round2(t *testing.T) {
	...
	for i, chain := range sourceChain {
		...
		mockReaders[chain] = contractreader3.NewMockExtended(t)
...
func TestCCIPChainReader_DiscoverContracts_GetAllSourceChainConfig_Errors(t *testing.T) {
	...
	ccipChainReader := &ccipChainReader{
		destChain: destChain,
		contractReaders: map[cciptypes.ChainSelector]contractreader2.Extended{
			destChain: destExtended,
			...
			sourceChain1: contractreader3.NewMockExtended(t),
			sourceChain2: contractreader3.NewMockExtended(t),
		},
		lggr:         logger.Test(t),
		configPoller: mockCache,
	}
...
func TestCCIPChainReader_DiscoverContracts_GetOfframpStaticConfig_Errors(t *testing.T) {
	...
	ccipChainReader := &ccipChainReader{
		destChain: destChain,
		contractReaders: map[cciptypes.ChainSelector]contractreader2.Extended{
			destChain: contractreader3.NewMockExtended(t),
			// these won't be used in this test, but are needed because
			// we determine the source chain selectors to query from the chains
			// that we have readers for.
			sourceChain1: contractreader3.NewMockExtended(t),
...
func TestCCIPChainReader_getFeeQuoterTokenPriceUSD(t *testing.T) {
	...
	ccipReader, err := newCCIPChainReaderInternal(
		...
		chainAccessors,
		map[cciptypes.ChainSelector]contractreader2.ContractReaderFacade{
			chainC: contractreader3.NewMockContractReaderFacade(t),
...
func TestCCIPFeeComponents_HappyPath(t *testing.T) {
	...
	sourceCRs := make(map[cciptypes.ChainSelector]*contractreader3.MockContractReaderFacade)
	for _, chain := range []cciptypes.ChainSelector{chainA, chainB, chainC} {
		sourceCRs[chain] = contractreader3.NewMockContractReaderFacade(t)
	}
...
func TestCCIPChainReader_LinkPriceUSD(t *testing.T) {
	...
	destCR := contractreader3.NewMockExtended(t)
...
func TestCCIPChainReader_Nonces(t *testing.T) {
	type testCase struct {
		...
		matchedBy      func(req contractreader2.ExtendedBatchGetLatestValuesRequest) bool
		mockResults    []*uint64
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			destReader := contractreader3.NewMockContractReaderFacade(t)
			cw := writermocks.NewMockContractWriter(t)
			contractWriters := make(map[cciptypes.ChainSelector]types.ContractWriter)
			contractWriters[chainB] = cw
...
func TestCCIPChainReader_DiscoverContracts_Parallel(t *testing.T) {
	...
	mockReaders := make(map[cciptypes.ChainSelector]*contractreader3.MockExtended)
	contractReaders := make(map[cciptypes.ChainSelector]contractreader2.Extended)
...
func TestCCIPChainReader_GetWrappedNativeTokenPriceUSD(t *testing.T) {
	...
	t.Run("happy path - gets prices for all chains", func(t *testing.T) {
		...
		ccipReader, err := newCCIPChainReaderWithConfigPollerInternal(
			...
			chainAccessors,
			map[cciptypes.ChainSelector]contractreader2.ContractReaderFacade{
				sourceChain1: contractreader3.NewMockContractReaderFacade(t),
				sourceChain2: contractreader3.NewMockContractReaderFacade(t),
			},
			contractWriters,
			destChain,
			offRampAddress,
...
func TestCCIPChainReader_GetChainFeePriceUpdate(t *testing.T) {
	...
	t.Run("happy path", func(t *testing.T) {
		...
		ccipReader, err := newCCIPChainReaderInternal(
			...
			chainAccessors,
			map[cciptypes.ChainSelector]contractreader2.ContractReaderFacade{
				chainB: contractreader3.NewMockContractReaderFacade(t),
			},
			contractWriters,
			chainB,
			offRampAddress,
	...
	t.Run("empty selectors", func(t *testing.T) {
		mockReader := contractreader3.NewMockExtended(t)
		ccipReader := &ccipChainReader{
			lggr:      lggr,
			destChain: destChain,
			contractReaders: map[cciptypes.ChainSelector]contractreader2.Extended{
	...
	t.Run("batch call error", func(t *testing.T) {
		...
		ccipReader, err := newCCIPChainReaderInternal(
			...
			chainAccessors,
			map[cciptypes.ChainSelector]contractreader2.ContractReaderFacade{
				destChain: contractreader3.NewMockContractReaderFacade(t),
			},
			contractWriters,
			destChain,
			offRampAddress,

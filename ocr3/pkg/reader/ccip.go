...
func newCCIPChainReaderWithConfigPollerInternal(
	...
	var crs = make(map[cciptypes.ChainSelector]contractreader2.Extended)
	for chainSelector, cr := range contractReaders {
		crs[chainSelector] = contractreader2.NewExtendedContractReader(cr)
	}
...
func (r *ccipChainReader) WithExtendedContractReaderTESTONLY(
	ch cciptypes.ChainSelector, cr contractreader2.Extended) *ccipChainReader {
...
func (r *ccipChainReader) fetchFreshSourceChainConfigs(
	...
	results, _, err := reader.ExtendedBatchGetLatestValues(
		ctx,
		contractreader2.ExtendedBatchGetLatestValuesRequest{
...
func (r *ccipChainReader) prepareBatchConfigRequests(
	chainSel cciptypes.ChainSelector) contractreader2.ExtendedBatchGetLatestValuesRequest {
	...
	var requests contractreader2.ExtendedBatchGetLatestValuesRequest

	// Only add OnRamp config requests if this is a source chain (not destination chain)
	if chainSel != r.destChain {
		requests = contractreader2.ExtendedBatchGetLatestValuesRequest{
...
type ccipReaderInternal interface {
	...
	getContractReader(chain cciptypes.ChainSelector) (contractreader2.Extended, bool)

	// prepareBatchConfigRequests prepares the batch requests for fetching chain configuration
	prepareBatchConfigRequests(chainSel cciptypes.ChainSelector) contractreader2.ExtendedBatchGetLatestValuesRequest
...
// getContractReader returns the contract reader for the specified chain
func (r *ccipChainReader) getContractReader(chain cciptypes.ChainSelector) (contractreader2.Extended, bool) {

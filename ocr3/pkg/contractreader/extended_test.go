...
import (
	...
	chainreadermocks "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/pkg/contractreader"
...
func TestExtendedContractReader(t *testing.T) {
	...
	extCr := contractreader2.NewExtendedContractReader(cr)
...
func TestDoubleWrap(t *testing.T) {
	var cr contractreader2.ContractReaderFacade

	wrapped := contractreader2.NewExtendedContractReader(cr)
	require.NotEqual(t, &cr, &wrapped)

	doubleWrapped := contractreader2.NewExtendedContractReader(cr)
...
func TestFinalityViolation(t *testing.T) {
	...
	_, err := wrapped.QueryKey(
		...
		query.LimitAndSort{},
		nil)
	require.ErrorIs(t, err, contractreader2.ErrFinalityViolated)

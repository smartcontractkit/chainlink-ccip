...
var _ plugincommon2.PluginProcessor[dt.Query, dt.Observation, dt.Outcome] = &ContractDiscoveryProcessor{}
...
type ContractDiscoveryProcessor struct {
	lggr            logger.Logger
	reader          *reader2.CCIPReader
	homechain       reader2.HomeChain
...
func NewContractDiscoveryProcessor(
	...
	reporter plugincommon2.MetricsReporter,
) plugincommon2.PluginProcessor[dt.Query, dt.Observation, dt.Outcome] {
	...
	return plugincommon2.NewTrackedProcessor(lggr, p, "discovery", reporter)
}
...
func (cdp *ContractDiscoveryProcessor) ValidateObservation(
	_ dt.Outcome, _ dt.Query, ao plugincommon2.AttributedObservation[dt.Observation],
) error {
	...
	err = plugincommon2.ValidateFChain(ao.Observation.FChain)
	if err != nil {
...
func (cdp *ContractDiscoveryProcessor) Outcome(
	ctx context.Context, _ dt.Outcome, _ dt.Query, aos []plugincommon2.AttributedObservation[dt.Observation],
) (dt.Outcome, error) {
	lggr := logutil.WithContextValues(ctx, cdp.lggr)
	lggr.Debugw("Processing contract discovery outcome")
	contracts := make(reader2.ContractAddresses)
	...
	donThresh := consensus2.MakeConstantThreshold[cciptypes.ChainSelector](consensus2.TwoFPlus1(cdp.fRoleDON))
	fChain := consensus2.GetConsensusMap(lggr, "fChain", agg.fChain, donThresh)
	fChainThresh := consensus2.MakeMultiThreshold(fChain, consensus2.TwoFPlus1)
	...
	if _, exists := fChain[cdp.dest]; !exists {
		...
		destThresh := consensus2.MakeConstantThreshold[cciptypes.ChainSelector](consensus2.TwoFPlus1(fChain[cdp.dest]))

		onrampConsensus := consensus2.GetConsensusMap(
			lggr,
			"onramp",
			agg.onrampAddrs,
			destThresh,
	...
	nonceManagerConsensus := consensus2.GetConsensusMap(
		lggr,
		"nonceManager",
	...
	rmnRemoteConsensus := consensus2.GetConsensusMap(
		lggr,
		"rmnRemote",
		agg.rmnRemoteAddrs,
		fChainThresh,
	...
	feeQuoterConsensus := consensus2.GetConsensusMap(
		lggr,
		"fee quoter",
		agg.feeQuoterAddrs,
		fChainThresh,
	...
	routerConsensus := consensus2.GetConsensusMap(
		lggr,
		"router",
		agg.routerAddrs,
		fChainThresh,
...
func (cdp *ContractDiscoveryProcessor) syncContracts(lggr logger.Logger, contracts reader2.ContractAddresses) {
...
func (s *readerSyncer) Sync(ctx context.Context, contracts reader2.ContractAddresses) (alreadySyncing bool, err error) {

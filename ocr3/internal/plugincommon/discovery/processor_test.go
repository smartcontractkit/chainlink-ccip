...
import (
	...
	mock_home_chain "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/internal_/reader"
	mock_reader "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/pkg/reader"
...
func TestContractDiscoveryProcessor_Observation_SupportsDest_HappyPath(t *testing.T) {
	...
	mockReaderIface := reader2.CCIPReader(mockReader)
	...
	expectedContracts := reader2.ContractAddresses{
...
func TestContractDiscoveryProcessor_Outcome_HappyPath(t *testing.T) {
	...
	mockReader.
		...
		Run(func(_ context.Context, _ reader2.ContractAddresses) { close(syncCalled) }).
		Return(nil)
	...
	aos := []plugincommon2.AttributedObservation[discoverytypes.Observation]{
		{Observation: obsSrc},
		{Observation: obsSrc},
		{Observation: obsSrc},
	}
...
func TestContractDiscoveryProcessor_ValidateObservation_HappyPath(t *testing.T) {
	...
	ao := plugincommon2.AttributedObservation[discoverytypes.Observation]{
		OracleID:    oracleID,
...
func TestContractDiscoveryProcessor_ValidateObservation_OracleNotAllowedToObserve(t *testing.T) {
	...
	cases := []struct {
		...
		addresses       reader2.ContractAddresses
...
func internalNewContractDiscoveryProcessor(
...
) plugincommon2.PluginProcessor[discoverytypes.Query, discoverytypes.Observation, discoverytypes.Outcome] {
	return NewContractDiscoveryProcessor(
		...
		oracleIDToP2PID,
		oracleID,
		plugincommon2.NoopReporter{},
	)
}

func TestReaderSyncer_Sync_FirstCall(t *testing.T) {
	mockReader := mock_reader.NewMockCCIPReader(t)
	var readerInstance reader2.CCIPReader = mockReader
	syncer := &readerSyncer{reader: &readerInstance}

	contracts := reader2.ContractAddresses{}

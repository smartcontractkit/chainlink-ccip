...
func TestObservationSize(t *testing.T) {
	...
	commitObs := make(exectypes2.CommitObservations, estimatedMaxNumberOfSourceChains)
	...
	for i := 0; i < maxCommitReports; i++ {
		...
		commitObs[idx] = append(commitObs[idx], exectypes2.CommitData{
	...
	msgObs := make(exectypes2.MessageObservations, estimatedMaxNumberOfSourceChains)
	...
	tokenDataObs := make(exectypes2.TokenDataObservations, estimatedMaxNumberOfSourceChains)
	for i := 0; i < maxMessages; i++ {
		idx := ccipocr3.ChainSelector(i % estimatedMaxNumberOfSourceChains)
		if nil == tokenDataObs[idx] {
			tokenDataObs[idx] = make(map[ccipocr3.SeqNum]exectypes2.MessageTokenData)
		}
		data := make([]byte, tokenDataSize)
		tokenDataObs[idx][bigSeqNum+ccipocr3.SeqNum(i)] = exectypes2.MessageTokenData{
	...
	noncesObs := make(exectypes2.NonceObservations, maxMessages)
	...
	encSize := func(obs exectypes2.Observation) int {
	...
	fmt.Printf("%d CommitReports: %d\n", len(maxObs.CommitReports),
		encSize(exectypes2.Observation{CommitReports: commitObs}))
	fmt.Printf("%d Messages: %d\n", msgSum, encSize(exectypes2.Observation{Messages: msgObs}))
	fmt.Printf("TokenData: %d\n", encSize(exectypes2.Observation{TokenData: tokenDataObs}))
	fmt.Printf("Nonces: %d\n", encSize(exectypes2.Observation{Nonces: noncesObs}))
	fmt.Printf("Contracts: %d\n", encSize(exectypes2.Observation{Contracts: discoveryObs}))

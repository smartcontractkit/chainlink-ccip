...
import (
	...
	consensus2 "github.com/smartcontractkit/chainlink-ccip/ocr3/internal/plugincommon/consensus"
	dt "github.com/smartcontractkit/chainlink-ccip/ocr3/internal/plugincommon/discovery/discoverytypes"
	ocrtypecodec "github.com/smartcontractkit/chainlink-ccip/ocr3/pkg/ocrtypecodec/v1"
...
func validateCommitReportsReadingEligibility(
	...
	case exectypes2.Filter, exectypes2.Initialized, exectypes2.Unknown:
		// nothing to validate
		break
	}
...
func validateTokenDataObservations(
	observedMsgs exectypes2.MessageObservations,
	tokenData exectypes2.TokenDataObservations,
) error {
...
func validateHashesExist(
	observedMsgs exectypes2.MessageObservations,
	hashes exectypes2.MessageHashes,
) error {
...
func validateObservedSequenceNumbers(observedData map[cciptypes.ChainSelector][]exectypes2.CommitData) error {
...
func computeRanges(reports []exectypes2.CommitData) ([]cciptypes.SeqNumRange, error) {
...
) exectypes2.CommitObservations {
	commitReportCache := make(map[cciptypes.ChainSelector][]exectypes2.CommitData)
...
func combineReportsAndMessages(
	reports []exectypes2.CommitData, executedMessages []cciptypes.SeqNum,
) (pending []exectypes2.CommitData, fullyExecuted []exectypes2.CommitData) {
...
) ([]plugincommon.AttributedObservation[exectypes2.Observation], error) {
	decoded := make([]plugincommon.AttributedObservation[exectypes2.Observation], len(aos))
	for i, ao := range aos {
		observation, err := ocrTypeCodec.DecodeObservation(ao.Observation)
		if err != nil {
			return nil, err
		}
		decoded[i] = plugincommon.AttributedObservation[exectypes2.Observation]{
...
func computeMessageObservationsConsensus(
	lggr logger.Logger,
	aos []plugincommon.AttributedObservation[exectypes2.Observation],
	fChain map[cciptypes.ChainSelector]int,
) exectypes2.MessageObservations {
	...
	results := make(exectypes2.MessageObservations)
...
func prepareValidatorsForComputeMessageObservationsConsensus(
	lggr logger.Logger,
	observations []plugincommon.AttributedObservation[exectypes2.Observation],
	fChain map[cciptypes.ChainSelector]int,
) map[cciptypes.ChainSelector]consensus2.OracleMinObservation[cciptypes.Message] {
	// Create a validator for each chain
	validators := make(map[cciptypes.ChainSelector]consensus2.OracleMinObservation[cciptypes.Message])
	for selector, f := range fChain {
		validators[selector] = consensus2.NewOracleMinObservation[cciptypes.Message](consensus2.FPlus1(f), nil)
	}
...
func computeCommitObservationsConsensus(
	...
	for mr, votes := range merkleRootsVotes {
		if consensus2.GteFPlusOne(fChain[mr.SourceChain], votes) {
	...
	result := make(exectypes2.CommitObservations)
	for _, mr := range validRoots {
		...
		for seqNum, count := range executedMsgVotes[mr] {
			if consensus2.LtFPlusOne(f, count) {
		...
		if _, ok := result[mr.SourceChain]; !ok {
			result[mr.SourceChain] = make([]exectypes2.CommitData, 0)
		}
		...
		result[mr.SourceChain] = append(result[mr.SourceChain], exectypes2.CommitData{
...
func computeMessageHashesConsensus(
	...
	results := make(exectypes2.MessageHashes)
	...
	for chainSeqNum, hashesAndVotes := range hashVotes {
		...
		for hash, numVotes := range hashesAndVotes {
			if consensus2.LtFPlusOne(f, numVotes) {
...
func computeTokenDataObservationsConsensus(
	...
	aos []plugincommon.AttributedObservation[exectypes2.Observation],
	fChain map[cciptypes.ChainSelector]int,
) exectypes2.TokenDataObservations {
	// Single message can transfer multiple tokens, so we need to find consensus on the token level.
	validators :=
		make(map[cciptypes.ChainSelector]map[reader.MessageTokenID]consensus2.OracleMinObservation[exectypes2.TokenData])
	results := make(exectypes2.TokenDataObservations)
	...
	for _, ao := range aos {
		for selector, seqMap := range ao.Observation.TokenData {
			...
			if consensus2.LtFPlusOne(f, numChainObservations[selector]) {
			...
			if _, ok1 := results[selector]; !ok1 {
				results[selector] = make(map[cciptypes.SeqNum]exectypes2.MessageTokenData)
			}

			if _, ok1 := validators[selector]; !ok1 {
				validators[selector] = make(map[reader.MessageTokenID]consensus2.OracleMinObservation[exectypes2.TokenData])
			}
	...
	for selector, seqNrs := range validators {
		for tokenID, validator := range seqNrs {
			...
			if values := validator.GetValid(); len(values) == 1 {
				...
				mtd = mtd.Append(tokenID.Index, exectypes2.NotReadyToken())
			}
...
func initResultsAndValidators(
	...
	f int,
	seqMap map[cciptypes.SeqNum]exectypes2.MessageTokenData,
	results exectypes2.TokenDataObservations,
	validators map[cciptypes.ChainSelector]map[reader.MessageTokenID]consensus2.OracleMinObservation[exectypes2.TokenData],
	...
	for seqNr, msgTokenData := range seqMap {
		...
		if consensus2.LtFPlusOne(f, cnt) {
			continue
		}

		if _, ok := results[selector][seqNr]; !ok {
			results[selector][seqNr] = exectypes2.NewMessageTokenData()
		}
		...
		for tokenIndex, tokenData := range msgTokenData.TokenData {
			...
			if _, ok := validators[selector][messageTokenID]; !ok {
				validators[selector][messageTokenID] =
					consensus2.NewOracleMinObservation(consensus2.FPlus1(f), exectypes2.TokenDataHash)
			}
...
func computeNoncesConsensus(
	...
	consensusNonces := make(exectypes2.NonceObservations, len(observedNonces))
...
func computeConsensusObservation(
	...
	destChain cciptypes.ChainSelector,
	f int,
) (exectypes2.Observation, error) {
	fChain := getConsensusFChain(lggr, observations, f)

	destFChain, ok := fChain[destChain]
	if !ok {
		return exectypes2.Observation{}, fmt.Errorf("destination chain %d is not in FChain %v", destChain, fChain)
	}

	consensusObservation := exectypes2.NewObservation(
...
func getConsensusFChain(
	...
	donThresh := consensus2.MakeConstantThreshold[cciptypes.ChainSelector](consensus2.TwoFPlus1(f))
	fChain := consensus2.GetConsensusMap(lggr, "fChain", observedFChains, donThresh)

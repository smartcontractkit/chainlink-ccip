...
import (
	...
	testhelpersrand "github.com/smartcontractkit/chainlink-ccip/ocr3/internal/libs/testhelpers/rand"
	mocks2 "github.com/smartcontractkit/chainlink-ccip/ocr3/internal/mocks"
	gasmock "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/chainlink_common/ccipocr3"
)
...
func assertMerkleRoot(
	...
	commitReport exectypes2.CommitData,
) {
...
func makeTestCommitReport(
	...
	messageTokenData := make([]exectypes2.MessageTokenData, numMessages)
	for i := 0; i < len(messages); i++ {
		messageTokenData[i] = exectypes2.MessageTokenData{
...
func setMessageData(
	idx int, size uint64, commitReport exectypes2.CommitData,
) exectypes2.CommitData {
...
func Test_buildSingleChainReport_Errors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				report: exectypes2.CommitData{
					MessageTokenData:    make([]exectypes2.MessageTokenData, 2),
		...
		},
		{
			...
			args: args{
				report: exectypes2.CommitData{
					MessageTokenData:    make([]exectypes2.MessageTokenData, 1),
					SourceChain:         1111,
					SequenceNumberRange: cciptypes.NewSeqNumRange(cciptypes.SeqNum(100), cciptypes.SeqNum(100)),
					Messages: []cciptypes.Message{
						{
...
func Test_Builder_Build(t *testing.T) {
	hasher := mocks2.NewMessageHasher()
	codec := mocks2.NewExecutePluginJSONReportCodec()
	lggr := logger.Test(t)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var updatedMessages []exectypes2.CommitData
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.fields.encoder != nil {
				...
				resolvedEncoder = mocks2.NewExecutePluginJSONReportCodec()
			}
...
func Test_execReportBuilder_checkMessage(t *testing.T) {
	type args struct {
		idx        int
		nonces     map[cciptypes.ChainSelector]map[string]uint64
		execReport exectypes2.CommitData
	}
	tests := []struct {
		name             string
		args             args
		expectedData     exectypes2.CommitData
			...
			args: args{
				...
				execReport: exectypes2.CommitData{
					...
					MessageTokenData: []exectypes2.MessageTokenData{
						{TokenData: []exectypes2.TokenData{{Ready: false, Data: nil}}},
					},
				},
			},
		...
		},
		{
			...
			args: args{
				...
				execReport: exectypes2.CommitData{
					...
					MessageTokenData: []exectypes2.MessageTokenData{
						{TokenData: []exectypes2.TokenData{{Ready: true, Data: []byte{0x01, 0x02, 0x03}}}},
					},
				},
			},
		...
		},
		{
			...
			args: args{
				...
				execReport: exectypes2.CommitData{
					...
					MessageTokenData: []exectypes2.MessageTokenData{
						{TokenData: []exectypes2.TokenData{{Ready: true, Data: []byte{}}}},
					},
				},
			},
			expectedStatus: ReadyToExecute,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if reflect.DeepEqual(tt.expectedData, exectypes2.CommitData{}) {

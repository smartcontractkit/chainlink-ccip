...
import (
	...
	cache2 "github.com/smartcontractkit/chainlink-ccip/ocr3/execute/internal/cache"
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/internal/plugincommon/discovery"
...
type ContractDiscoveryInterface plugincommon2.PluginProcessor[dt.Query, dt.Observation, dt.Outcome]
...
type Plugin struct {
	...
	commitRootsCache     cache2.CommitsRootsCache
	commitReportCache    cache2.CommitReportCache
...
func NewPlugin(
	...
	commitReportCacheCfg := cache2.CommitReportCacheConfig{
		MessageVisibilityInterval: offchainCfg.MessageVisibilityInterval.Duration(),
		EvictionGracePeriod:       cache2.EvictionGracePeriod,
		CleanupInterval:           cache2.CleanupInterval,
	}
	...
	p := &Plugin{
		...
		chainSupport: plugincommon2.NewChainSupport(
		...
		commitRootsCache: cache2.NewCommitRootsCache(
		...
		commitReportCache: cache2.NewCommitReportCache(
			logutil.WithComponent(lggr, "CommitReportCache"),
			commitReportCacheCfg,
			&cache2.RealTimeProvider{},
			ccipReader,
		),
		inflightMessageCache: cache2.NewInflightMessageCache(offchainCfg.InflightCacheExpiry.Duration()),
...
func getSortedExecutableReports(lggr logger.Logger,
	...
	canExecute CanExecuteHandle) []exectypes2.CommitData {
	// Filter out reports that cannot be executed (executed or snoozed).
	var executableReports []exectypes2.CommitData
	{
	...
	sort.Slice(executableReports, func(i, j int) bool {
		return exectypes2.LessThan(executableReports[i], executableReports[j])
	})
...
func getExecutableReportRanges(lggr logger.Logger,
	groupedCommits exectypes2.CommitObservations,
	canExecute CanExecuteHandle,
) (
	map[cciptypes.ChainSelector][]cciptypes.SeqNumRange,
	map[cciptypes.ChainSelector][]exectypes2.CommitData,
	error) {
	rangesBySelector := make(map[cciptypes.ChainSelector][]cciptypes.SeqNumRange)
	executableReports := make(map[cciptypes.ChainSelector][]exectypes2.CommitData)
...
func removeUnconfirmedAndFinalizedMessages(
	executableReports map[cciptypes.ChainSelector][]exectypes2.CommitData,
	finalizedMessages, unconfirmedMessages map[cciptypes.ChainSelector][]cciptypes.SeqNum,
) (
	remainingReportsBySelector map[cciptypes.ChainSelector][]exectypes2.CommitData,
	fullyExecutedFinalized []exectypes2.CommitData,
	fullyExecutedUnfinalized []exectypes2.CommitData,
) {
	remainingReportsBySelector = make(map[cciptypes.ChainSelector][]exectypes2.CommitData)
...
func getPendingReportsForExecution(
	...
	candidateReports := cache2.DeduplicateReports(lggr, append(reportsFromCache, unfinalizedIncrementReports...))
...
func (p *Plugin) ValidateObservation(
	...
	var previousOutcome exectypes2.Outcome
	...
	if nextState == exectypes2.GetCommitReports {
	...
	if nextState == exectypes2.GetMessages || nextState == exectypes2.Filter {
...
func validateCommonStateObservations(
	...
	decodedObservation exectypes2.Observation,
	supportedChains mapset.Set[cciptypes.ChainSelector],
	state exectypes2.PluginState,
) error {
	if err := plugincommon2.ValidateFChain(decodedObservation.FChain); err != nil {
	...
	if p.discovery != nil {
		aos := plugincommon2.AttributedObservation[dt.Observation]{
			OracleID:    oracleID,
...
) ([]cciptypes.ExecutePluginReport, [][]exectypes2.CommitData, error) {
...
func extractReportInfo(report exectypes2.Outcome) ([]cciptypes.ExecuteReportInfo, error) {
...
func (p *Plugin) validateReport(
	...
	if r.Report == nil {
		...
		return cciptypes.ExecutePluginReport{}, plugincommon2.NewErrInvalidReport("nil report")
	}

	decodedReport, err = p.reportCodec.Decode(ctx, r.Report)
	if err != nil {
		return cciptypes.ExecutePluginReport{},
			plugincommon2.NewErrValidatingReport(fmt.Errorf("decode exec plugin report: %w", err))
	}

	if len(decodedReport.ChainReports) == 0 {
		lggr.Infow("skipping empty report")
		return cciptypes.ExecutePluginReport{},
			plugincommon2.NewErrInvalidReport("empty report")
	}
	...
	if err != nil {
		return cciptypes.ExecutePluginReport{},
			plugincommon2.NewErrValidatingReport(fmt.Errorf("supports dest chain: %w", err))
	}
	...
	if !supports {
		...
		return cciptypes.ExecutePluginReport{},
			plugincommon2.NewErrInvalidReport("dest chain not supported")
	}

	offRampConfigDigest, err := p.ccipReader.GetOffRampConfigDigest(ctx, consts.PluginTypeExecute)
	if err != nil {
		return cciptypes.ExecutePluginReport{},
			plugincommon2.NewErrValidatingReport(fmt.Errorf("get offramp config digest: %w", err))
	}
	...
	if !bytes.Equal(offRampConfigDigest[:], p.reportingCfg.ConfigDigest[:]) {
		...
		return cciptypes.ExecutePluginReport{},
			plugincommon2.NewErrInvalidReport("offramp config digest mismatch")
	}
	...
	if errors.Is(err, errAlreadyExecuted) {
		...
		return decodedReport, plugincommon2.NewErrInvalidReport(err.Error())
	}
	if err != nil {
		...
		return decodedReport, plugincommon2.NewErrValidatingReport(err)
	}
...
func (p *Plugin) ShouldAcceptAttestedReport(
	...
	if !supportsDest {
		lggr.Errorw("dest chain not supported by this oracle, can't run report acceptance procedures, " +
			...
			"assigned as readers of the destination chain, or if " +
			"this oracle should support the destination chain but isn't!")
		return false, plugincommon2.NewErrInvalidReport("destination chain not supported")
	}

	decodedReport, err := p.validateReport(ctx, lggr, r)
	if errors.Is(err, plugincommon2.ErrInvalidReport) {
	...
	isCursed, err := plugincommon2.IsReportCursed(ctx, lggr, p.ccipReader, sourceChains)
	if err != nil {
		lggr.Errorw(

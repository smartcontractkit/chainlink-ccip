...
func (p *Plugin) Observation(
	...
	if previousOutcome.State == exectypes2.Filter {
	...
	if p.discovery != nil {
		...
		if !p.contractsInitialized {
			...
			return p.ocrTypeCodec.EncodeObservation(exectypes2.Observation{Contracts: discoveryObs, FChain: fChain})
		}
	}
...
func (p *Plugin) getCommitReportsObservation(
	...
	if err != nil {
		return exectypes2.Observation{}, fmt.Errorf("unable to determine if the destination chain is supported: %w", err)
	}
	...
	if err != nil {
		return exectypes2.Observation{}, err
	}
...
func (p *Plugin) observeTokenDataForMessage(
	...
	msgObs := exectypes2.MessageObservations{
		srcChain: {
			seqNum: msg,
		},
	}
	...
	if err != nil {
		...
		msgTkData = make(exectypes2.TokenDataObservations)
		msgTkData[srcChain] = make(map[cciptypes.SeqNum]exectypes2.MessageTokenData)
		msgTkData[srcChain][seqNum] = exectypes2.NewMessageTokenData(exectypes2.NewErrorTokenData(err))
	}
...
func (p *Plugin) getMessagesObservation(
	...
	messageObs := make(exectypes2.MessageObservations)
	availableReports := make(exectypes2.CommitObservations)
	msgHashes := make(exectypes2.MessageHashes)
	tkData := make(exectypes2.TokenDataObservations)

	sort.Slice(commitData, func(i, j int) bool {
		return exectypes2.LessThan(commitData[i], commitData[j])
	})

	supportedChains, err := p.supportedChains(p.reportingCfg.OracleID)
	if err != nil {
		return exectypes2.Observation{}, fmt.Errorf("get supported chains: %w", err)
	}
	...
	for _, report := range commitData {
		...
		if _, ok := messageObs[srcChain]; !ok {
			...
			tkData[srcChain] = make(map[cciptypes.SeqNum]exectypes2.MessageTokenData)
		}
		...
		for _, msg := range msgs {
			...
			tkData[srcChain][seqNum] = exectypes2.NewMessageTokenData()
			hash, err := p.msgHasher.Hash(ctx, msg)
			if err != nil {
				return exectypes2.Observation{}, fmt.Errorf(
		...
		for _, msg := range msgs {
			...
			if err != nil {
				return exectypes2.Observation{}, fmt.Errorf("unable to encode observation: %w", err)
			}
			...
			if totalMsgs > lenientMaxMsgsPerObs || encodedObsSize > lenientMaxObservationLength {
				...
				observation.TokenData[srcChain][seqNum] = exectypes2.NewMessageTokenData()
				stop = true
				break
			}
		}

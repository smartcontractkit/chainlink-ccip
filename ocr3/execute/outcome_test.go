...
func TestOutcome_PseudoDeletedConsensusConflict(t *testing.T) {
	...
	tkData1 := exectypes2.NewMessageTokenData(exectypes2.NewSuccessTokenData([]byte{1, 1, 1, 1, 1, 1}))
	tkData2 := exectypes2.NewMessageTokenData(exectypes2.NewSuccessTokenData([]byte{1, 1, 1, 1, 1, 2}))

	oracleObservations := make(map[commontypes.OracleID]exectypes2.Observation)
	...
	for _, oracleID := range []uint8{1, 2, 3, 4} {
		obs := exectypes2.Observation{
			...
			TokenData: map[cciptypes.ChainSelector]map[cciptypes.SeqNum]exectypes2.MessageTokenData{
				srcChain1: {
		...
		if oracleID == 3 || oracleID == 4 {
			...
			obs.TokenData[srcChain2][srcChain2SeqNr] = exectypes2.NewMessageTokenData()
		}
		oracleObservations[commontypes.OracleID(oracleID)] = obs
	}

	aos := make([]plugincommon.AttributedObservation[exectypes2.Observation], 0)
	for oracleID, observation := range oracleObservations {
		aos = append(aos, plugincommon.AttributedObservation[exectypes2.Observation]{

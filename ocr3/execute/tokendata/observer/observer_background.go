...
func (o *backgroundObserver) Observe(
	...
	tokenDataResults := make(exectypes2.TokenDataObservations)
	...
	for chainSel, seqNumToMsg := range observations {
		tokenDataResults[chainSel] = make(map[cciptypes.SeqNum]exectypes2.MessageTokenData)
		for seqNum, msg := range seqNumToMsg {
			tokenDataResults[chainSel][seqNum] = exectypes2.MessageTokenData{
				TokenData: make([]exectypes2.TokenData, len(msg.TokenAmounts)),
			}

			for i := range msg.TokenAmounts {
				tokenDataResults[chainSel][seqNum].TokenData[i] = exectypes2.TokenData{
					Ready:     false,
...
type inMemTokenDataCache struct {
	...
	inMemTokenData     map[cciptypes.Bytes32]exectypes2.MessageTokenData
...
func newInMemObservationsCache(lggr logger.Logger,
	...
	c := &inMemTokenDataCache{
		...
		inMemTokenData:     make(map[cciptypes.Bytes32]exectypes2.MessageTokenData),
...
func (c *inMemTokenDataCache) get(msgID cciptypes.Bytes32) (exectypes2.MessageTokenData, bool) {
	c.mu.RLock()
	defer c.mu.RUnlock()

	msgData, ok := c.inMemTokenData[msgID]
	if !ok {
		return exectypes2.MessageTokenData{}, false
	}
...
func (c *inMemTokenDataCache) set(msgID cciptypes.Bytes32, tokenData exectypes2.MessageTokenData) {
	c.mu.Lock()
	defer c.mu.Unlock()

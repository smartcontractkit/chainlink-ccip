...
func assertTokenDataEmptyInitially(t *testing.T,
	msgObservations exectypes2.MessageObservations, tokenDataObservations exectypes2.TokenDataObservations) {
	...
	for chain, seqNums := range tokenDataObservations {
		...
		for _, tokenData := range seqNums {
			for _, td := range tokenData.TokenData {
				require.Equal(t, exectypes2.TokenData{Supported: true}, td)
			}
		}
	}
}
...
func generateMsgObservations(
	numMsgsPerChain map[cciptypes.ChainSelector]int,
) (exectypes2.MessageObservations, map[cciptypes.ChainSelector][]cciptypes.SeqNum) {

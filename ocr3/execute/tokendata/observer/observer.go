...
func (c *compositeTokenDataObserver) Observe(
	ctx context.Context,
	msgObservations exectypes2.MessageObservations,
) (exectypes2.TokenDataObservations, error) {
...
func (c *compositeTokenDataObserver) initTokenDataObservations(
	...
	tokenObservation := make(exectypes2.TokenDataObservations)
	for chainSelector, observation := range observations {
		tokenObservation[chainSelector] = make(map[cciptypes.SeqNum]exectypes2.MessageTokenData)
		...
		for seq, message := range observation {
			...
			for i, token := range message.TokenAmounts {
				if !c.IsTokenSupported(chainSelector, token) {
					...
					tokenData[i] = exectypes2.NewNoopTokenData()
					continue
				}
				...
				tokenData[i] = exectypes2.TokenData{Ready: false, Supported: true}
			}
			tokenObservation[chainSelector][seq] = exectypes2.MessageTokenData{TokenData: tokenData}
		}
	}
...
func merge(
	lggr logger.Logger,
	base exectypes2.TokenDataObservations,
	from exectypes2.TokenDataObservations,
) (exectypes2.TokenDataObservations, error) {
...
func (n *NoopTokenDataObserver) Observe(
	...
	for selector, obs := range observations {
		tokenObservations[selector] = make(map[cciptypes.SeqNum]exectypes2.MessageTokenData)

		for seq, message := range obs {
			tokenData := make([]exectypes2.TokenData, len(message.TokenAmounts))
			for i := range message.TokenAmounts {
				if n.isError(selector, seq, i) {
					tokenData[i] = exectypes2.NewErrorTokenData(errors.New("some error"))
				} else {
					tokenData[i] = exectypes2.NewNoopTokenData()
				}
			}
			tokenObservations[selector][seq] = exectypes2.MessageTokenData{TokenData: tokenData}
		}
	}

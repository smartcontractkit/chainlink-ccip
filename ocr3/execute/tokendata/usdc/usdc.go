...
type USDCTokenDataObserver struct {
	...
	usdcMessageReader        reader2.USDCMessageReader
...
func NewUSDCTokenDataObserver(
	...
	usdcReader, err := reader2.NewUSDCMessageReader(
		ctx,
		lggr,
		usdcConfig.Tokens,
		readers,
...
) map[cciptypes.ChainSelector]map[reader2.MessageTokenID]cciptypes.RampTokenAmount {
	usdcMessages := make(map[cciptypes.ChainSelector]map[reader2.MessageTokenID]cciptypes.RampTokenAmount)
	for chainSelector, messages := range messageObservations {
		usdcMessages[chainSelector] = make(map[reader2.MessageTokenID]cciptypes.RampTokenAmount)
		for seqNum, message := range messages {
			for i, tokenAmount := range message.TokenAmounts {
				...
				if isUSDC {
					usdcMessages[chainSelector][reader2.NewMessageTokenID(seqNum, i)] = tokenAmount
				}
				lggr.Debugw(
...
func (u *USDCTokenDataObserver) fetchUSDCEventMessages(
	ctx context.Context,
	lggr logger.Logger,
	usdcMessages map[cciptypes.ChainSelector]map[reader2.MessageTokenID]cciptypes.RampTokenAmount,
) (map[cciptypes.ChainSelector]map[reader2.MessageTokenID]cciptypes.Bytes, error) {
	output := make(map[cciptypes.ChainSelector]map[reader2.MessageTokenID]cciptypes.Bytes)
...
func (u *USDCTokenDataObserver) fetchAttestations(
	ctx context.Context,
	usdcMessages map[cciptypes.ChainSelector]map[reader2.MessageTokenID]cciptypes.Bytes,
) (map[cciptypes.ChainSelector]map[reader2.MessageTokenID]tokendata.AttestationStatus, error) {
...
func (u *USDCTokenDataObserver) extractTokenData(
	...
	lggr logger.Logger,
	messages exectypes2.MessageObservations,
	attestations map[cciptypes.ChainSelector]map[reader2.MessageTokenID]tokendata.AttestationStatus,
) (exectypes2.TokenDataObservations, error) {
	...
	for chainSelector, chainMessages := range messages {
		...
		for seqNum, message := range chainMessages {
			tokenData := make([]exectypes2.TokenData, len(message.TokenAmounts))
			for i, tokenAmount := range message.TokenAmounts {
			...
			tokenObservations[chainSelector][seqNum] = exectypes2.NewMessageTokenData(tokenData...)
		}
	}
...
func (u *USDCTokenDataObserver) attestationToTokenData(
	...
	tokenIndex int,
	attestations map[reader2.MessageTokenID]tokendata.AttestationStatus,
) exectypes2.TokenData {
	status, ok := attestations[reader2.NewMessageTokenID(seqNr, tokenIndex)]
	if !ok {
		return exectypes2.NewErrorTokenData(tokendata.ErrDataMissing)
	}
	if status.Error != nil {
		return exectypes2.NewErrorTokenData(status.Error)
	}
	tokenData, err := u.attestationEncoder(ctx, status.MessageBody, status.Attestation)
	if err != nil {
		return exectypes2.NewErrorTokenData(fmt.Errorf("unable to encode attestation: %w", err))
	}
	return exectypes2.NewSuccessTokenData(tokenData)
}

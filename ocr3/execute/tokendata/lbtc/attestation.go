...
import (
	...
	tokendata2 "github.com/smartcontractkit/chainlink-ccip/ocr3/execute/tokendata"
	"github.com/smartcontractkit/chainlink-ccip/ocr3/execute/tokendata/http"
...
func NewLBTCAttestationClient(
	lggr logger.Logger,
	config pluginconfig.LBTCObserverConfig,
) (tokendata2.AttestationClient, error) {
	...
	return tokendata2.NewObservedAttestationClient(
...
func (c *LBTCAttestationClient) Attestations(
...
) (map[cciptypes.ChainSelector]map[reader.MessageTokenID]tokendata2.AttestationStatus, error) {
	attestations := make(map[string]tokendata2.AttestationStatus)
	...
	res := make(map[cciptypes.ChainSelector]map[reader.MessageTokenID]tokendata2.AttestationStatus)
	for chainSelector, tokenDatas := range messages {
		res[chainSelector] = make(map[reader.MessageTokenID]tokendata2.AttestationStatus)
...
func (c *LBTCAttestationClient) fetchBatch(
...
) (map[string]tokendata2.AttestationStatus, error) {
	...
	if err != nil {
		for _, inputMessageHash := range messageHashes {
			attestations[inputMessageHash] = tokendata2.ErrorAttestationStatus(err)
		}
	...
	if attestationResp.Code != 0 {
		for _, inputMessageHash := range messageHashes {
			attestations[inputMessageHash] = tokendata2.ErrorAttestationStatus(
...
func attestationToAttestationStatus(attestation Attestation) tokendata2.AttestationStatus {
	if attestation.Status == attestationStatusSubmitted || attestation.Status == attestationStatusPending {
		return tokendata2.ErrorAttestationStatus(tokendata2.ErrNotReady)
	}
	if attestation.Status == attestationStatusFailed || attestation.Status == attestationStatusUnspecified {
		return tokendata2.ErrorAttestationStatus(tokendata2.ErrUnknownResponse)
	}
	payloadHashBytes, err := cciptypes.NewBytesFromString(attestation.MessageHash)
	if err != nil {
		return tokendata2.ErrorAttestationStatus(fmt.Errorf("failed to decode message hash in attestation: %w", err))
	}
	attestationBytes, err := cciptypes.NewBytesFromString(attestation.Data)
	if err != nil {
		return tokendata2.ErrorAttestationStatus(fmt.Errorf("failed to decode attestation: %w", err))
	}
	return tokendata2.SuccessAttestationStatus(payloadHashBytes, []byte{}, attestationBytes)
}

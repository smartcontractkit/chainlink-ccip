...
import (
	...
	codec_mocks "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/chainlink_common/ccipocr3"
	plugincommon2 "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/internal_/plugincommon"
	reader_mock "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/internal_/reader"
	readerpkg_mock "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/pkg/reader"
...
func Test_getPendingReportsForExecution(t *testing.T) {
	...
	tcs := []struct {
		...
		fetchFrom               time.Time
		wantObs                 exectypes2.CommitObservations
		wantExecutedFinalized   []exectypes2.CommitData
		wantExecutedUnfinalized []exectypes2.CommitData
		...
		},
		{
			...
			fetchFrom:               time.Time{},
			wantObs:                 exectypes2.CommitObservations{}, // Empty observations since all chains are cursed
			wantExecutedFinalized:   nil,
			wantExecutedUnfinalized: nil,
			wantErr:                 assert.NoError,
		},
		...
		},
		{
			...
			wantObs: exectypes2.CommitObservations{
				1: []exectypes2.CommitData{ // Expecting only the report with the Merkle root to be processed
					{
						SourceChain:         1,
						SequenceNumberRange: cciptypes.NewSeqNumRange(1, 10),
						Timestamp:           time.UnixMilli(10101010103),
	...
	for _, tt := range tcs {
		t.Run(tt.name, func(t *testing.T) {
			...
			var mockCache cache2.CommitReportCache = &noopCommitReportCache{}
...
func TestPlugin_ValidateObservation_IneligibleMessageObserver(t *testing.T) {
	...
	observation := exectypes2.NewObservation(nil, exectypes2.MessageObservations{
	...
	prevOutcome := exectypes2.Outcome{
...
func TestPlugin_ValidateObservation_IneligibleCommitReportsObserver(t *testing.T) {
	...
	commitReports := map[cciptypes.ChainSelector][]exectypes2.CommitData{
		1: {
			{
	...
	observation := exectypes2.NewObservation(
...
func TestPlugin_ValidateObservation_CallsDiscoveryValidateObservation(t *testing.T) {
	...
	mockHomeChain := reader_mock.NewMockHomeChain(t)
	mockHomeChain.EXPECT().GetSupportedChainsForPeer(mock.Anything).Return(mapset.NewSet(cciptypes.ChainSelector(1)), nil)
	mockDiscoveryProcessor := plugincommon2.NewMockPluginProcessor[dt.Query, dt.Observation, dt.Outcome](t)
...
func TestPlugin_Outcome_DestFChainNotAvailable(t *testing.T) {
	...
	observation, err := ocrTypeCodec.EncodeObservation(exectypes2.Observation{
...
func TestPlugin_Reports_MultipleReports(t *testing.T) {
	...
	mockChainSupport := plugincommon2.NewMockChainSupport(t)
	...
	outcome := exectypes2.NewOutcome(exectypes2.Filter, commitReports, reports)
...
func TestPlugin_Reports_UnableToEncode(t *testing.T) {
	...
	// Mock chain support for transmission schedule
	chainSupport := plugincommon2.NewMockChainSupport(t)
	...
	report, err := ocrTypeCodec.EncodeOutcome(exectypes2.NewOutcome(
		exectypes2.Unknown, []exectypes2.CommitData{{}, {}},
...
func TestPlugin_ShouldAcceptAttestedReport_DoesNotDecode(t *testing.T) {
	...
	cs := plugincommon2.NewMockChainSupport(t)
...
func TestCommitRootsCache_SkippedRootScenario(t *testing.T) {
	...
	// Create the cache
	cache := cache2.NewCommitRootsCache(
		lggr,
...
func (c *configurableCommitReportCache) DeduplicateReports(
	...
	if c.lggr != nil {
		return cache2.DeduplicateReports(c.lggr, reports)
	}

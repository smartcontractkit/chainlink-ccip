...
type PluginFactory struct {
	...
	extendedReaders   map[cciptypes.ChainSelector]contractreader2.Extended
...
func (p *PluginFactory) NewReportingPlugin(ctx context.Context, config ocr3types.ReportingPluginConfig,
	...
	readerFacades := make(map[cciptypes.ChainSelector]contractreader2.ContractReaderFacade, len(p.extendedReaders))
	for chain, cr := range p.extendedReaders {
		readerFacades[chain] = cr
	}

	// Bind the RMNHome contract
	var rmnHomeReader reader2.RMNHome
	if offchainConfig.RMNEnabled {
		...
		rmnHomeReader, err = reader2.NewRMNHomeChainReader(
			ctx,
			lggr,
			reader2.HomeChainPollingInterval,
			p.homeChainSelector,
			rmnHomeAddress,
			rmnCr,
		)
	...
	ccipReader, err := reader2.NewCCIPChainReader(
		ctx,
	...
	onChainTokenPricesReader := reader2.NewPriceReader(
...
func validateOcrConfig(cfg reader2.OCR3Config) error {

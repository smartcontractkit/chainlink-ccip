...
import (
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/commit/tokenprice"
	...
	reader3 "github.com/smartcontractkit/chainlink-ccip/ocr3/mocks/pkg/reader"
...
func TestPlugin_E2E_AllNodesAgree_MerkleRoots(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			runner := testhelpers2.NewOCR3Runner(nodes, oracleIDs, encodedPrevOutcome)
...
func TestPlugin_E2E_AllNodesAgree_TokenPrices(t *testing.T) {
	...
	testCases := []struct {
		...
		prevOutcome           committypes.Outcome
		expOutcome            committypes.Outcome
		mockPriceReader       func(*reader3.MockPriceReader)
		expTransmittedReports []ccipocr3.CommitPluginReport
		enableDiscovery       bool
	...
	}{
		{
			...
			prevOutcome: committypes.Outcome{},
			mockPriceReader: func(m *reader3.MockPriceReader) {
				m.EXPECT().
...
func TestPlugin_E2E_AllNodesAgree_ChainFee(t *testing.T) {
	...
	expectedChain1FeeOutcome := chainfee2.Outcome{
	...
	testCases := []struct {
		...
		mockCCIPReader func(*reader3.MockCCIPReader)
	}{
		{
			...
			expOutcome: committypes.Outcome{
				MerkleRootOutcome: merkleOutcome,
				ChainFeeOutcome: chainfee2.Outcome{
					GasPrices: []ccipocr3.GasPriceChain{
						{
							GasPrice: evmPackedGasPrice,
							ChainSel: sourceEvmChain1,
			...
			mockCCIPReader: func(m *reader3.MockCCIPReader) {
				m.EXPECT().
...
func prepareCcipReaderMock(
	ccipReader *reader3.MockCCIPReader,
...
func preparePriceReaderMock(priceReader *reader3.MockPriceReader) {
	priceReader.EXPECT().
		GetFeeQuoterTokenUpdates(mock.Anything, mock.Anything, mock.Anything).
		Return(
			nil, nil,
...
type nodeSetup struct {
	node        *Plugin
	ccipReader  *reader3.MockCCIPReader
	priceReader *reader3.MockPriceReader
	reportCodec *mocks2.CommitPluginJSONReportCodec
	msgHasher   *mocks2.MessageHasher
}
...
func setupNode(params SetupNodeParams) nodeSetup {
	ccipReader := reader3.NewMockCCIPReader(params.t)
	tokenPricesReader := reader3.NewMockPriceReader(params.t)
	reportCodec := mocks2.NewCommitPluginJSONReportCodec()
	msgHasher := mocks2.NewMessageHasher()
	homeChainReader := reader_mock.NewMockHomeChain(params.t)
	rmnHomeReader := reader3.NewMockRMNHome(params.t)
...
func defaultNodeParams(t *testing.T) SetupNodeParams {
	...
	rmnRemoteCfg := testhelpers2.CreateRMNRemoteCfg()
...
func newRandomFees(selector ccipocr3.ChainSelector) (components types.ChainFeeComponents,
	...
	usdPricesF := chainfee2.FeeComponentsToPackedFee(chainfee2.ComponentsUSDPrices{

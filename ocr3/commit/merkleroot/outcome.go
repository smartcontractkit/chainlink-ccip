...
func buildMerkleRootsOutcome(
	...
	if len(roots) > 0 && rmnEnabled {
		sigs, err = rmn2.NewECDSASigsFromPB(rmnSigs.Signatures)
		if err != nil {
...
func getConsensusObservation(
	...
	donThresh := consensus2.MakeConstantThreshold[cciptypes.ChainSelector](consensus2.TwoFPlus1(fRoleDON))
	...
	fChain := consensus2.MakeMultiThreshold(fChains, consensus2.F)
	...
	consensusObs := consensusObservation{
		MerkleRoots:      consensus2.GetConsensusMap(lggr, "Merkle Root", aggObs.MerkleRoots, twoFChainPlus1),
		RMNEnabledChains: consensus2.GetConsensusMap(lggr, "RMNEnabledChains", aggObs.RMNEnabledChains, twoFChainPlus1),
		OnRampMaxSeqNums: consensus2.GetOrderedConsensus(
			lggr,
			"OnRamp Max Seq Nums",
		...
		RMNRemoteConfig:    consensus2.GetConsensusMap(lggr, "RMNRemote cfg", rmnRemoteConfigs, twoFChainPlus1),
		FChain:             fChains,
	}

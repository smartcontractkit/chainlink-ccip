...
import (
	...
	reader2 "github.com/smartcontractkit/chainlink-ccip/ocr3/internal/reader"
	...
	reader3 "github.com/smartcontractkit/chainlink-ccip/ocr3/pkg/reader"
...
func Test_maxObservationLength(t *testing.T) {
	...
	maxObs := committypes.Observation{
		...
		DiscoveryObs: dt.Observation{
			FChain:    make(map[ccipocr3.ChainSelector]int, estimatedMaxNumberOfSourceChains),
			Addresses: make(reader3.ContractAddresses, estimatedMaxNumberOfSourceChains*maxContractsPerChain),
		},
		FChain: make(map[ccipocr3.ChainSelector]int, estimatedMaxNumberOfSourceChains),
	}
...
func TestPluginFactory_NewReportingPlugin(t *testing.T) {
	t.Run("basic checks for the happy flow", func(t *testing.T) {
		...
		p := &PluginFactory{
			baseLggr: lggr,
			ocrConfig: reader3.OCR3ConfigWithMeta{
				Version:      1,

...
import (
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/internal/libs/mathslib"
...
func (p *processor) getConsensusObservation(
	...
	donThresh := consensus2.MakeConstantThreshold[cciptypes.ChainSelector](consensus2.TwoFPlus1(p.fRoleDON))
	fChains := consensus2.GetConsensusMap(lggr, "fChain", aggObs.FChain, donThresh)
	...
	if consensus2.LtTwoFPlusOne(fDestChain, len(aggObs.Timestamps)) {
		return Observation{},
			fmt.Errorf("not enough observations for timestamps to reach consensus, have %d, need %d",
				len(aggObs.Timestamps), consensus2.TwoFPlus1(fDestChain))
	}
	timestamp := consensus2.Median(aggObs.Timestamps, consensus2.TimestampComparator)

	chainFeeUpdatesConsensus := consensus2.GetConsensusMapAggregator(
		lggr,
		"ChainFeeUpdates",
		aggObs.ChainFeeUpdates,
		consensus2.MakeConstantThreshold[cciptypes.ChainSelector](consensus2.TwoFPlus1(fDestChain)),
		chainFeeUpdateAggregator,
	)

	twoFChainPlus1 := consensus2.MakeMultiThreshold(fChains, consensus2.TwoFPlus1)

	feeComponents := consensus2.GetConsensusMapAggregator(
		lggr,
		"FeeComponents",
		...
		func(vals []types.ChainFeeComponents) types.ChainFeeComponents {
			...
			return types.ChainFeeComponents{
				ExecutionFee:        consensus2.Median(executionFees, consensus2.BigIntComparator).Int,
				DataAvailabilityFee: consensus2.Median(dataAvailabilityFees, consensus2.BigIntComparator).Int,
			}
		},
	)

	nativeTokenPrices := consensus2.GetConsensusMapAggregator(
		lggr,
		"NativeTokenPrices",
		...
		func(vals []cciptypes.BigInt) cciptypes.BigInt {
			return consensus2.Median(vals, consensus2.BigIntComparator)
		},
	)
...
func chainFeeUpdateAggregator(updates []Update) Update {
	...
	medianExecFeeUSD := consensus2.Median(execFeeUSDs, func(a, b *big.Int) bool {
		return a.Cmp(b) == -1
	})
	medianDataAvFeeUSD := consensus2.Median(dataAvFeeUSDs, func(a, b *big.Int) bool {
		return a.Cmp(b) == -1
	})
	...
	return Update{
		...
		Timestamp: consensus2.TimestampsMedian(timestamps),
	}
}

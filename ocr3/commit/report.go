...
import (
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/commit/merkleroot"
	...
	"github.com/smartcontractkit/chainlink-ccip/ocr3/internal/plugincommon/consensus"
...
func (p *Plugin) validateReport(
	...
	if err != nil {
		return cciptypes.CommitPluginReport{},
			plugincommon2.NewErrValidatingReport(fmt.Errorf("decode report: %w, report: %x", err, r.Report))
	}

	var reportInfo cciptypes.CommitReportInfo
	if reportInfo, err = cciptypes.DecodeCommitReportInfo(r.Info); err != nil {
		return cciptypes.CommitPluginReport{},
			plugincommon2.NewErrValidatingReport(fmt.Errorf("decode report info: %w", err))
	}
	...
	for _, root := range decodedReport.BlessedMerkleRoots {
		if root.MerkleRoot == (cciptypes.Bytes32{}) {
			...
			return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("empty blessed merkle root")

		}
		if root.SeqNumsRange.Start() > root.SeqNumsRange.End() {
			lggr.Warnw("invalid seqNumsRange", "blessed root", root)
			return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("invalid seqNumsRange")
		}
	}
	...
	for _, root := range decodedReport.UnblessedMerkleRoots {
		if root.MerkleRoot == (cciptypes.Bytes32{}) {
			...
			return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("empty unblessed merkle root")
		}
		if root.SeqNumsRange.Start() > root.SeqNumsRange.End() {
	...
	for _, sig := range decodedReport.RMNSignatures {
		...
		if _, ok := seen[sig]; ok {
			...
			return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("duplicate RMN signature")
		}
		seen[sig] = struct{}{}
	}
	...
	if p.offchainCfg.RMNEnabled &&
		...
		return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("insufficient RMN signatures")
	}

	if isCursed, err := p.checkReportCursed(ctx, lggr, decodedReport); err != nil || isCursed {
		if err != nil {
			return cciptypes.CommitPluginReport{},
				plugincommon2.NewErrValidatingReport(fmt.Errorf("check report cursed: %w", err))
		}
		return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("report cursed")
	}
	...
	if !supports {
		...
		return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("dest chain not supported")
	}

	offRampConfigDigest, err := p.ccipReader.GetOffRampConfigDigest(ctx, consts.PluginTypeCommit)
	if err != nil {
		err = plugincommon2.NewErrValidatingReport(fmt.Errorf("get offramp config digest: %w", err))
		return cciptypes.CommitPluginReport{}, plugincommon2.NewErrValidatingReport(err)
	}
	...
	if !bytes.Equal(offRampConfigDigest[:], p.reportingCfg.ConfigDigest[:]) {
		...
		return cciptypes.CommitPluginReport{}, plugincommon2.NewErrInvalidReport("config digest mismatch")
	}

	err = p.isStaleReport(ctx, seqNr, decodedReport)
	if err != nil {
		return cciptypes.CommitPluginReport{}, plugincommon2.NewErrStaleReport(fmt.Sprintf("%v", err))
	}
	...
	if err != nil {
		...
		err = plugincommon2.NewErrInvalidReport(fmt.Sprintf("root blessings validation: %v", err))
...
func (p *Plugin) ShouldAcceptAttestedReport(
	...
	if errors.Is(err, plugincommon2.ErrStaleReport) {

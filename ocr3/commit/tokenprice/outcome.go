...
func (p *processor) getConsensusObservation(
	...
	feedPricesConsensus := consensus2.GetConsensusMapAggregator(
		lggr,
		"FeedTokenPrices",
		aggObs.FeedTokenPrices,
		consensus2.MakeConstantThreshold[cciptypes.UnknownEncodedAddress](consensus2.TwoFPlus1(fFeedChain)),
		func(vals []cciptypes.TokenPrice) cciptypes.TokenPrice {
			return consensus2.Median(vals, consensus2.TokenPriceComparator)
		},
	)

	feeQuoterUpdatesConsensus := consensus2.GetConsensusMapAggregator(
		lggr,
		"FeeQuoterUpdates",
		aggObs.FeeQuoterTokenUpdates,
		consensus2.MakeConstantThreshold[cciptypes.UnknownEncodedAddress](consensus2.TwoFPlus1(fDestChain)),
		// each key will have one object with the median for timestamps as timestamp value
		// and the median prices as price value
		consensus2.TimestampedBigAggregator,
	)

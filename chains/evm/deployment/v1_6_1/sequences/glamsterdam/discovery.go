// Package glamsterdam contains the sequences backing the v1.6 "update gas config for
...
type DiscoverLanesToTargetInput struct {
	...
	// FeeQuoterAddressByChain maps each candidate source chain selector to its v1.6 FeeQuoter
	// address on that chain. Chains whose FeeQuoter address couldn't be resolved from the
	// datastore should be omitted here and reported separately by the caller.
...
// FeeQuoter has an enabled DestChainConfig pointed at the target chain — i.e. whether that chain
// has a lane sending into the Glamsterdam target chain and therefore needs its source-side gas
// config updated. This is a read-only scan; it makes one RPC call per candidate chain.
var DiscoverLanesToTarget = cldf_ops.NewSequence(
	"DiscoverLanesToTargetV1_6",

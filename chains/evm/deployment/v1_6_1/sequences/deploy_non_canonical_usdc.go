...
var DeployNonCanonicalUSDC = cldf_ops.NewSequence(
	...
	func(b cldf_ops.Bundle, dep adapters.DeployCCTPChainDeps, input adapters.DeployCCTPInput) (output sequences.OnChainOutput, err error) {
		...
		if len(existingPools) == 1 {
			...
			burnMintWithLockReleaseFlagTokenPoolReport, err := evmops.ExecuteDeploy(b, burn_mint_with_lock_release_flag_token_pool.Deploy, chain, contract_utils.DeployInput[burn_mint_with_lock_release_flag_token_pool.ConstructorArgs]{

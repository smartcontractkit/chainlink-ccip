...
var ConfigureTokenPool = cldf_ops.NewSequence(
	...
	func(b cldf_ops.Bundle, chain evm.Chain, input ConfigureTokenPoolInput) (output sequences.OnChainOutput, err error) {
		...
		if len(input.AllowList) != 0 {
			allowListEnabledReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewReadGetAllowListEnabled(bound), chain, cldf_contract.FunctionInput[struct{}]{Args: struct{}{}})
			if err != nil {
			...
			if allowListEnabledReport.Output {
				...
				currentAllowListReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewReadGetAllowList(bound), chain, cldf_contract.FunctionInput[struct{}]{Args: struct{}{}})
				if err != nil {
				...
				if len(adds) != 0 || len(removes) != 0 {
					applyAllowListUpdatesReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewWriteApplyAllowListUpdates(bound), chain, cldf_contract.FunctionInput[token_pool.ApplyAllowListUpdatesArgs]{
		...
		if input.RouterAddress != (common.Address{}) {
			currentRouterReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewReadGetRouter(bound), chain, cldf_contract.FunctionInput[struct{}]{Args: struct{}{}})
			if err != nil {
			...
			if currentRouterReport.Output != input.RouterAddress {
				setRouterReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewWriteSetRouter(bound), chain, cldf_contract.FunctionInput[common.Address]{
		...
		if input.RateLimitAdmin != (common.Address{}) {
			currentRateLimitAdminReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewReadGetRateLimitAdmin(bound), chain, cldf_contract.FunctionInput[struct{}]{Args: struct{}{}})
			if err != nil {
			...
			if currentRateLimitAdminReport.Output != input.RateLimitAdmin {
				setRateLimitAdminReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewWriteSetRateLimitAdmin(bound), chain, cldf_contract.FunctionInput[common.Address]{

...
var ConfigureTokenForTransfers = cldf_ops.NewSequence(
	...
	func(b cldf_ops.Bundle, chains chain.BlockChains, input tokens.ConfigureTokenForTransfersInput) (output sequences.OnChainOutput, err error) {
		...
		tp, err := gbtp.NewTokenPool(tokenPoolAddress, chain.Client)
		if err != nil {
		...
		if input.TokenAddress != "" {
			...
			tokenAddrReport, err := cldf_ops.ExecuteOperation(b, token_pool.NewReadGetToken(tp), chain, cldf_contract.FunctionInput[struct{}]{Args: struct{}{}})
			if err != nil {
		...
		isSupported, err := cldf_ops.ExecuteOperation(b, token_pool.NewReadIsSupportedToken(tp), chain, cldf_contract.FunctionInput[common.Address]{
			Args: tokenAddress,
		})
		if err != nil {

...
func TestUpdateLockReleasePoolAddressesChangeset(t *testing.T) {
	...
	lockReleasePoolAddress := common.HexToAddress("0x1234567890123456789012345678901234567890")
		...
		ChainInputs: []changesets.UpdateLockReleasePoolAddressesPerChainInput{
			{
				...
				LockReleasePoolAddrs: usdc_token_pool_proxy.UpdateLockReleasePoolAddressesArgs{
					...
					LockReleasePools:     []common.Address{lockReleasePoolAddress},
				},
			},
		},
		MCMS: mcms.Input{
	...
	output, err := updateLockReleasePoolAddressesChangeset.Apply(*e, updateLockReleasePoolAddressesInput)
	...
	require.Equal(t, lockReleasePoolAddr, lockReleasePoolAddress, "Lock release pool address should match")
}

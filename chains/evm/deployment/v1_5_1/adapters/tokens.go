...
// RemoveRemotePool returns a sequence that removes remote pool entries from a v1.5.1 token
// pool. It reads the current on-chain remote pools for each remote chain and returns an
// error when a requested remote pool is not configured, rather than emitting a
...
func (t *TokenAdapter) RemoveRemotePool() *cldf_ops.Sequence[tokensapi.RemoveRemotePoolSequenceInput, sequences.OnChainOutput, cldf_chain.BlockChains] {
	return cldf_ops.NewSequence(
		"evm-v1.5.1-adapter:remove-remote-pool",
		tpOps.Version,
		"Removes remote pool entries from a v1.5.1 token pool on an EVM chain",
...
func (p *poolOpsV151) SetAdmins(b cldf_ops.Bundle, chain evm.Chain, poolAddr common.Address, rlAdmin, feeAdmin *common.Address) ([]evm_contract.WriteOutput, error) {
	if feeAdmin != nil {
	...
	pool, err := token_pool.NewTokenPool(poolAddr, chain.Client)
	if err != nil {
		return nil, fmt.Errorf("failed to instantiate v1.5.1 token pool contract at %s: %w", poolAddr.Hex(), err)
	}

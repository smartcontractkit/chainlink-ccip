...
type ConfigImportAdapter struct {
	OnRamp        map[uint64]map[uint64]common.Address
	OffRamp       map[uint64]map[uint64]common.Address
...
func (ci *ConfigImportAdapter) ConnectedChains(_ cldf.Environment, chainsel uint64) ([]uint64, error) {
	onRamps, ok := ci.OnRamp[chainsel]
	if !ok {
		return nil, fmt.Errorf("no onramps found for chain %d", chainsel)
	}
	var connected []uint64
	for destSel := range onRamps {
...
func (ci *ConfigImportAdapter) SupportedTokensPerRemoteChain(e cldf.Environment, chainsel uint64) (map[uint64][]common.Address, error) {
	...
	return GetSupportedTokensPerRemoteChain(tokenAdminRegAddr, chain)
}
		...
		func(b cldf_ops.Bundle, chains cldf_chain.BlockChains, in api.ImportConfigPerChainInput) (output sequences.OnChainOutput, err error) {
			...
			result, err = sequences.RunAndMergeSequence(b, chains,
				...
				seq1_5.OnRampImportConfigSequenceInput{
					...
					OnRampsPerRemoteChain:   ci.OnRamp[chainSelector],
			...
			result, err = sequences.RunAndMergeSequence(b, chains,
				...
				seq1_5.OffRampImportConfigSequenceInput{
					ChainSelector:          chainSelector,
					OffRampsPerRemoteChain: ci.OffRamp[chainSelector],
				}, result)
			if err != nil {

...
var ManualRegistrationSequence = operations.NewSequence(
	...
	tarops.Version,
	"Set or transfer token administrators on the TokenAdminRegistry contract across multiple EVM chains",
	func(b operations.Bundle, chains cldf_chain.BlockChains, input ManualRegistrationSequenceInput) (sequences.OnChainOutput, error) {
		...
		// Manual registration behavior varies depending on whether an administrator is already set on the token
		// admin registry. If an administrator is not set, then we'll propose a new administrator. Otherwise, we
		// transfer the admin role to a new administrator. This makes the logic idempotent allowing it to be run

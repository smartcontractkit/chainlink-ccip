...
func deployDiscoveryFeeQuoter(
	...
	e *cldf.Environment,
	chainSel uint64,
	destChainConfigArgs []fq20ops.DestChainConfigArgs,
) common.Address {
	t.Helper()

	chain := e.BlockChains.EVMChains()[chainSel]
	out, err := cldf_ops.ExecuteOperation(e.OperationsBundle, fq20ops.Deploy, chain, contract.DeployInput[fq20ops.ConstructorArgs]{
...
func TestDiscoverLanesToTarget(t *testing.T) {
	...
	enabledDestChainConfig := fq20ops.DestChainConfig{
		IsEnabled:                true,
	...
	laneAAddr := deployDiscoveryFeeQuoter(t, e, discoveryLaneChainSelA, []fq20ops.DestChainConfigArgs{
		{
			DestChainSelector: discoveryTargetChainSel,
			DestChainConfig:   enabledDestChainConfig,
		},
	})
	laneBAddr := deployDiscoveryFeeQuoter(t, e, discoveryLaneChainSelB, []fq20ops.DestChainConfigArgs{
		{
			DestChainSelector: discoveryTargetChainSel,
			DestChainConfig:   enabledDestChainConfig,
		},

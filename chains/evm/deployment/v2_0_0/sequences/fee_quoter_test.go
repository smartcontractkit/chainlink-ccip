...
func TestMergeFeeQuoterUpdateOutputs(t *testing.T) {
	...
	t.Run("ConstructorArgs - merge DestChainConfig,PriceUpdaters TokenTransferFeeConfigArgs", func(t *testing.T) {
		output16 := sequences.FeeQuoterUpdate{
			ConstructorArgs: fqops.ConstructorArgs{
				...
				TokenTransferFeeConfigArgs: []fq_bindings.FeeQuoterTokenTransferFeeConfigArgs{
					{
						DestChainSelector: 100,
						TokenTransferFeeConfigs: []fq_bindings.FeeQuoterTokenTransferFeeConfigSingleTokenArgs{
							{Token: addr1},
						},
					},
				},
				DestChainConfigArgs: []fq_bindings.FeeQuoterDestChainConfigArgs{
					{
						DestChainSelector: 100,
	...
	t.Run("TokensToUseDefaultFeeConfigs - merge by DestChainSelector and Token", func(t *testing.T) {
		output16 := sequences.FeeQuoterUpdate{
			TokenTransferFeeConfigUpdates: fqops.ApplyTokenTransferFeeConfigUpdatesArgs{
				TokensToUseDefaultFeeConfigs: []fq_bindings.FeeQuoterTokenTransferFeeConfigRemoveArgs{
		...
		require.Contains(t, result.TokenTransferFeeConfigUpdates.TokensToUseDefaultFeeConfigs,
			fq_bindings.FeeQuoterTokenTransferFeeConfigRemoveArgs{DestChainSelector: 100, Token: addr1})
		require.Contains(t, result.TokenTransferFeeConfigUpdates.TokensToUseDefaultFeeConfigs,
			fq_bindings.FeeQuoterTokenTransferFeeConfigRemoveArgs{DestChainSelector: 100, Token: addr2})
		require.Contains(t, result.TokenTransferFeeConfigUpdates.TokensToUseDefaultFeeConfigs,
			fq_bindings.FeeQuoterTokenTransferFeeConfigRemoveArgs{DestChainSelector: 200, Token: addr3})
	})
	...
	t.Run("comprehensive merge", func(t *testing.T) {
		...
		output16 := sequences.FeeQuoterUpdate{
			...
			DestChainConfigs: []fq_bindings.FeeQuoterDestChainConfigArgs{
				{DestChainSelector: 200, DestChainConfig: fq_bindings.FeeQuoterDestChainConfig{IsEnabled: true}},
			},
			TokenTransferFeeConfigUpdates: fqops.ApplyTokenTransferFeeConfigUpdatesArgs{
				TokenTransferFeeConfigArgs: []fq_bindings.FeeQuoterTokenTransferFeeConfigArgs{
					{DestChainSelector: 400, TokenTransferFeeConfigs: []fq_bindings.FeeQuoterTokenTransferFeeConfigSingleTokenArgs{{Token: addr1}}},
				},
			},
			AuthorizedCallerUpdates: fq_bindings.AuthorizedCallersAuthorizedCallerArgs{
		...
		output15 := sequences.FeeQuoterUpdate{
			...
			DestChainConfigs: []fq_bindings.FeeQuoterDestChainConfigArgs{
				{DestChainSelector: 200, DestChainConfig: fq_bindings.FeeQuoterDestChainConfig{IsEnabled: false}}, // duplicate
				{DestChainSelector: 300, DestChainConfig: fq_bindings.FeeQuoterDestChainConfig{IsEnabled: true}},  // unique
			},
			TokenTransferFeeConfigUpdates: fqops.ApplyTokenTransferFeeConfigUpdatesArgs{
				TokenTransferFeeConfigArgs: []fq_bindings.FeeQuoterTokenTransferFeeConfigArgs{
					{DestChainSelector: 400, TokenTransferFeeConfigs: []fq_bindings.FeeQuoterTokenTransferFeeConfigSingleTokenArgs{{Token: addr2}}}, // duplicate
					{DestChainSelector: 500, TokenTransferFeeConfigs: []fq_bindings.FeeQuoterTokenTransferFeeConfigSingleTokenArgs{{Token: addr3}}}, // unique
				},
			},
			AuthorizedCallerUpdates: fq_bindings.AuthorizedCallersAuthorizedCallerArgs{
...
func destChainConfigsFromSelectors(selectors ...uint64) []fq_bindings.FeeQuoterDestChainConfigArgs {
	out := make([]fq_bindings.FeeQuoterDestChainConfigArgs, len(selectors))
	for i, s := range selectors {
		out[i] = fq_bindings.FeeQuoterDestChainConfigArgs{
			DestChainSelector: s,
			DestChainConfig:   fq_bindings.FeeQuoterDestChainConfig{IsEnabled: true},
		}
	}
	return out
}

func tokenTransferFeeConfigArgsFromSelectors(selectors ...uint64) []fq_bindings.FeeQuoterTokenTransferFeeConfigArgs {
	out := make([]fq_bindings.FeeQuoterTokenTransferFeeConfigArgs, len(selectors))
	for i, s := range selectors {
		out[i] = fq_bindings.FeeQuoterTokenTransferFeeConfigArgs{DestChainSelector: s}
	}
	return out
}
...
func TestBatchedInputForSequenceFeeQuoterUpdate(t *testing.T) {
	...
	t.Run("constructor dest chain configs within batch size stay in constructor and one apply batch", func(t *testing.T) {
		...
		input := sequences.FeeQuoterUpdate{
			ConstructorArgs: fqops.ConstructorArgs{
				DestChainConfigArgs: append([]fq_bindings.FeeQuoterDestChainConfigArgs(nil), cfgs...),
			},
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
	...
	t.Run("constructor dest chain configs over batch size keeps first batch in constructor rest in dest batches", func(t *testing.T) {
		...
		require.Equal(t, []fq_bindings.FeeQuoterDestChainConfigArgs{cfgs[8]}, got.DestChainConfigBatches[0])
		require.Nil(t, got.TokenTransferFeeConfigBatches)
		require.Nil(t, got.GasPriceUpdateBatches)
		require.Nil(t, got.TokenPriceUpdateBatches)
	})
	...
	t.Run("update DestChainConfigs only are batched by size 8", func(t *testing.T) {
		...
		cfgs := destChainConfigsFromSelectors(selectors...)
		input := sequences.FeeQuoterUpdate{
			DestChainConfigs: append([]fq_bindings.FeeQuoterDestChainConfigArgs(nil), cfgs...),
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
	...
	t.Run("constructor remainder and update DestChainConfigs are concatenated", func(t *testing.T) {
		...
		input := sequences.FeeQuoterUpdate{
			ConstructorArgs: fqops.ConstructorArgs{
				DestChainConfigArgs: append([]fq_bindings.FeeQuoterDestChainConfigArgs(nil), cons...),
			},
			DestChainConfigs: append([]fq_bindings.FeeQuoterDestChainConfigArgs(nil), updates...),
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
		require.Len(t, got.DestChainConfigBatches, 2)
		require.Equal(t, []fq_bindings.FeeQuoterDestChainConfigArgs{cons[sequences.DestChainConfigUpdateBatchLen]}, got.DestChainConfigBatches[0])
	...
	t.Run("constructor token transfer fee configs over batch size splits first batch into constructor", func(t *testing.T) {
		...
		input := sequences.FeeQuoterUpdate{
			ConstructorArgs: fqops.ConstructorArgs{
				TokenTransferFeeConfigArgs: append([]fq_bindings.FeeQuoterTokenTransferFeeConfigArgs(nil), args...),
			},
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
		require.Nil(t, got.DestChainConfigBatches)
		...
		require.Equal(t, []fq_bindings.FeeQuoterTokenTransferFeeConfigArgs{args[sequences.TokenTransferFeeConfigUpdateBatchLen]}, got.TokenTransferFeeConfigBatches[0])
		require.Nil(t, got.GasPriceUpdateBatches)
		require.Nil(t, got.TokenPriceUpdateBatches)
	})
	...
	t.Run("DestChainConfigs and TokenTransferFeeConfigUpdates batched together", func(t *testing.T) {
		...
		input := sequences.FeeQuoterUpdate{
			DestChainConfigs: append([]fq_bindings.FeeQuoterDestChainConfigArgs(nil), destCfgs...),
			TokenTransferFeeConfigUpdates: fqops.ApplyTokenTransferFeeConfigUpdatesArgs{
				TokenTransferFeeConfigArgs: append([]fq_bindings.FeeQuoterTokenTransferFeeConfigArgs(nil), tokenArgs...),
			},
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
	...
	t.Run("GasPriceUpdates within batch size returns single batch", func(t *testing.T) {
		gasUpdates := make([]fq_bindings.InternalGasPriceUpdate, sequences.GasPriceUpdateBatchLen)
		for i := range gasUpdates {
			gasUpdates[i] = fq_bindings.InternalGasPriceUpdate{
				DestChainSelector: uint64(i + 1),
				UsdPerUnitGas:     big.NewInt(int64(i + 1)),
			}
		}
		input := sequences.FeeQuoterUpdate{
			PriceUpdates: fq_bindings.InternalPriceUpdates{GasPriceUpdates: append([]fq_bindings.InternalGasPriceUpdate(nil), gasUpdates...)},
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
		require.Nil(t, got.DestChainConfigBatches)
		require.Nil(t, got.TokenTransferFeeConfigBatches)
	...
	t.Run("GasPriceUpdates over batch size split into batches of GasPriceUpdateBatchLen", func(t *testing.T) {
		...
		gasUpdates := make([]fq_bindings.InternalGasPriceUpdate, n)
		for i := range gasUpdates {
			gasUpdates[i] = fq_bindings.InternalGasPriceUpdate{
	...
	t.Run("TokenPriceUpdates within batch size returns single batch", func(t *testing.T) {
		tokenUpdates := make([]fq_bindings.InternalTokenPriceUpdate, sequences.TokenPriceUpdateBatchLen)
		for i := range tokenUpdates {
			tokenUpdates[i] = fq_bindings.InternalTokenPriceUpdate{
				SourceToken: common.BytesToAddress([]byte{byte(i + 1)}),
				UsdPerToken: big.NewInt(int64(i + 1)),
			}
		}
		input := sequences.FeeQuoterUpdate{
			PriceUpdates: fq_bindings.InternalPriceUpdates{TokenPriceUpdates: append([]fq_bindings.InternalTokenPriceUpdate(nil), tokenUpdates...)},
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
		require.Nil(t, got.DestChainConfigBatches)
		require.Nil(t, got.TokenTransferFeeConfigBatches)
	...
	t.Run("TokenPriceUpdates over batch size split into batches of TokenPriceUpdateBatchLen", func(t *testing.T) {
		...
		tokenUpdates := make([]fq_bindings.InternalTokenPriceUpdate, n)
		for i := range tokenUpdates {
			tokenUpdates[i] = fq_bindings.InternalTokenPriceUpdate{
	...
	t.Run("gas and token price batches both split for large inputs", func(t *testing.T) {
		...
		gasUpdates := make([]fq_bindings.InternalGasPriceUpdate, gasN)
		for i := range gasUpdates {
			gasUpdates[i] = fq_bindings.InternalGasPriceUpdate{DestChainSelector: uint64(i), UsdPerUnitGas: big.NewInt(1)}
		}
		tokenN := sequences.TokenPriceUpdateBatchLen + 3
		tokenUpdates := make([]fq_bindings.InternalTokenPriceUpdate, tokenN)
		for i := range tokenUpdates {
			tokenUpdates[i] = fq_bindings.InternalTokenPriceUpdate{SourceToken: common.BytesToAddress([]byte{byte(200 + i)}), UsdPerToken: big.NewInt(2)}
		}
		input := sequences.FeeQuoterUpdate{
			PriceUpdates: fq_bindings.InternalPriceUpdates{
				GasPriceUpdates:   append([]fq_bindings.InternalGasPriceUpdate(nil), gasUpdates...),
				TokenPriceUpdates: append([]fq_bindings.InternalTokenPriceUpdate(nil), tokenUpdates...),
			},
		}
		got := sequences.BatchedInputForSequenceFeeQuoterUpdate(&input)
		require.Len(t, got.GasPriceUpdateBatches, 2)

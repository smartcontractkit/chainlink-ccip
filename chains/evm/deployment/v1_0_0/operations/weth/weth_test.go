	...
	tests := []struct {
	...
	}{
		{
			...
			validate: func(t *testing.T, result mcms_types.BatchOperation) {
				...
				// Verify AdditionalFields contains the correct value (as number, not string)
				var additionalFields map[string]json.Number
				err := json.Unmarshal(tx.AdditionalFields, &additionalFields)
				require.NoError(t, err)
				require.Equal(t, validAmount.String(), additionalFields["value"].String())
			},
		},
		{
			name:          "zero amount",
		...
		},
		{
			...
			validate: func(t *testing.T, result mcms_types.BatchOperation) {
				...
				require.Equal(t, expectedAmount.String(), additionalFields["value"].String())
			},
		},
		{
	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, result mcms_types.Transaction) {
				...
				err := json.Unmarshal(result.AdditionalFields, &additionalFields)
				require.NoError(t, err)
				require.Equal(t, "1", additionalFields["value"].String())
			},
		},
		{
...
func TestAdditionalFieldsJSONStructure(t *testing.T) {
	...
	t.Run("CreateNativeETHTransfer JSON structure", func(t *testing.T) {
		...
		// Value should be the numeric representation of the amount
		require.Equal(t, "123456789", fields["value"].String())
	})

...
contract OffRampOverSuperchainInterop_constructProofs is OffRampOverSuperchainInteropSetup {
  function test_constructProofs() public view {
    ...
    (Identifier memory identifier, bytes32 logHash) = s_offRampHelper.constructProofs(message, proofs);
    ...
    // Verify log hash
    bytes32 expectedLogHash = keccak256(
      abi.encode(
  ...
  function testFuzz_constructProofs_Success(
    ...
    bytes32 expectedLogHash =
      keccak256(abi.encode(SuperchainInterop.SENT_MESSAGE_LOG_SELECTOR, DEST_CHAIN_SELECTOR, sequenceNumber, message));
  ...
  function test_constructProofs_RevertWhen_InvalidProofsLength() public {
    ...
    s_offRampHelper.constructProofs(message, shortProofs);
    ...
    s_offRampHelper.constructProofs(message, longProofs);
    ...
    s_offRampHelper.constructProofs(message, emptyProofs);
  }
}

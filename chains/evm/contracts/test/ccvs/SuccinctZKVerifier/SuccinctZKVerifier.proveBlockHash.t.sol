...
contract SuccinctZKVerifier_proveBlockHash is SuccinctZKVerifierSetup {
  ...
  function test_proveBlockHash() public {
    uint256 provenBlockNumber = PROVEN_BLOCK_NUMBER - HEADER_COUNT;
    ...
    s_zkVerifier.proveBlockHash(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER, s_headers);

    assertEq(provenBlockHash, s_zkVerifier.getProvenBlockHash(SOURCE_CHAIN_SELECTOR, provenBlockNumber));
    // Blocks proven by the light client are returned by the same getter.
    assertEq(keccak256(s_headers[0]), s_zkVerifier.getProvenBlockHash(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER));
  }

  function test_proveBlockHash_FromProvenBlock() public {
    bytes[] memory headers = new bytes[](1);
    headers[0] = s_headers[0];
    s_zkVerifier.proveBlockHash(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER, headers);

    headers[0] = s_headers[1];
    vm.expectEmit();
    emit SuccinctZKVerifier.BlockHashProven(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER - 2, keccak256(s_headers[2]));

    s_zkVerifier.proveBlockHash(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER - 1, headers);
  }
  ...
  function test_getProvenBlockHash_AfterLightClientReplaced() public {
    ...
    assertEq(bytes32(0), s_zkVerifier.getProvenBlockHash(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER - HEADER_COUNT));
  }
  ...
  function test_getProvenBlockHash_RevertWhen_SourceChainNotSupported() public {
    _setSourceChainConfig(MockSP1Helios(address(0)));

    vm.expectRevert(abi.encodeWithSelector(SuccinctZKVerifier.SourceChainNotSupported.selector, SOURCE_CHAIN_SELECTOR));
    s_zkVerifier.getProvenBlockHash(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER);
  }

  function test_proveBlockHash_RevertWhen_BlockNotProven() public {
    vm.expectRevert(
      abi.encodeWithSelector(SuccinctZKVerifier.BlockNotProven.selector, SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER - 1)
    );
    s_zkVerifier.proveBlockHash(SOURCE_CHAIN_SELECTOR, PROVEN_BLOCK_NUMBER - 1, s_headers);
  }

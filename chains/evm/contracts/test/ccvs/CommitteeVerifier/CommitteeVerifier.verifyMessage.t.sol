...
contract CommitteeVerifier_verifyMessage is CommitteeVerifierSetup {
  function test_verifyMessage_ExtractsSignatureLengthCorrectly() public view {
    ...
    bytes memory verifierResults = abi.encodePacked(s_versionTag, uint16(64), r, s);
  ...
  function test_verifyMessage_ForwardCompatibilityWithExtraData() public view {
    ...
    // Extra data can be added to the verifierResults without it affecting the signature validation.
    bytes memory extraFutureData = hex"0123456789012345678901234567890123456789012345678901234567890123";

    (bytes32 r, bytes32 s) = _signWithV27(PRIVATE_KEY_0, keccak256(bytes.concat(s_versionTag, messageHash)));
    bytes memory verifierResults = abi.encodePacked(s_versionTag, uint16(64), r, s, extraFutureData);
  ...
  function test_verifyMessage_RevertWhen_InvalidVerifierResults_InvalidPrefix() public {
    (MessageV1Codec.MessageV1 memory message,) = _generateBasicMessageV1();
    bytes32 messageHash = _generateMessageHash(message);

    // Create verifierResults with only 1 byte (missing signature length & verifier version).
    // Reverts because we need 2 bytes for uint16 & 4 bytes for verifier version.
    bytes memory verifierResults = hex"0102030405";

    // Should revert with InvalidVerifierResults when verifierResults is too short to contain signature length & verifier version.
    vm.expectRevert(abi.encodeWithSelector(CommitteeVerifier.InvalidVerifierResults.selector));
    s_committeeVerifier.verifyMessage(message, messageHash, verifierResults);
  }

  function test_verifyMessage_RevertWhen_InvalidVerifierResults_SignatureLengthExceedsVerifierResults() public {
    (MessageV1Codec.MessageV1 memory message,) = _generateBasicMessageV1();
    bytes32 messageHash = _generateMessageHash(message);

    // Set signature length to 100 but only provide 2 bytes of actual signature data.
    bytes memory verifierResults = abi.encodePacked(s_versionTag, uint16(100), hex"ab");

    // Should revert with InvalidVerifierResults when signature length exceeds available data.
    vm.expectRevert(abi.encodeWithSelector(CommitteeVerifier.InvalidVerifierResults.selector));
    s_committeeVerifier.verifyMessage(message, messageHash, verifierResults);
  }
  ...
  function test_verifyMessage_RevertWhen_InvalidVersion() public {
    ...
    bytes memory verifierResults = hex"010203040506";

...
contract LombardVerifier_removePaths is LombardVerifierSetup {
  function test_removePaths() public {
    ...
    assertEq(pathBefore.remoteBridgeSender, REMOTE_BRIDGE_SENDER);

    uint64[] memory chainsToRemove = new uint64[](1);
    chainsToRemove[0] = DEST_CHAIN_SELECTOR;

    vm.expectEmit();
    emit LombardVerifier.PathRemoved(DEST_CHAIN_SELECTOR, LOMBARD_CHAIN_ID, ALLOWED_CALLER, REMOTE_BRIDGE_SENDER);
    ...
    assertEq(pathAfter.remoteBridgeSender, bytes32(0));

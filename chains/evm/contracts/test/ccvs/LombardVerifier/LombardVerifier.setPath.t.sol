...
contract LombardVerifier_setPath is LombardVerifierSetup {
  ...
  bytes32 internal constant NEW_REMOTE_ADAPTER = bytes32("REMOTE_ADAPTER");

  function test_setPath() public {
    vm.expectEmit();
    emit LombardVerifier.PathSet(NEW_CHAIN_SELECTOR, NEW_LOMBARD_CHAIN_ID, NEW_ALLOWED_CALLER, NEW_REMOTE_ADAPTER);

    s_lombardVerifier.setPath(NEW_CHAIN_SELECTOR, NEW_LOMBARD_CHAIN_ID, NEW_ALLOWED_CALLER, NEW_REMOTE_ADAPTER);
    ...
    assertEq(path.remoteAdapter, NEW_REMOTE_ADAPTER);
  ...
  function test_setPath_RevertWhen_ZeroLombardChainId() public {
    ...
    s_lombardVerifier.setPath(NEW_CHAIN_SELECTOR, bytes32(0), NEW_ALLOWED_CALLER, bytes32(0));
  }

  function test_setPath_RevertWhen_ZeroAllowedCaller() public {
    vm.expectRevert(LombardVerifier.ZeroAllowedCaller.selector);
    s_lombardVerifier.setPath(NEW_CHAIN_SELECTOR, NEW_LOMBARD_CHAIN_ID, bytes32(0), bytes32(0));
  }
  ...
  function test_setPath_RevertWhen_OnlyCallableByOwner() public {
    ...
    s_lombardVerifier.setPath(NEW_CHAIN_SELECTOR, NEW_LOMBARD_CHAIN_ID, NEW_ALLOWED_CALLER, bytes32(0));
  }
}

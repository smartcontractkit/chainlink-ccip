...
contract CCTPVerifier_withdrawFeeTokens is CCTPVerifierSetup {
  ...
  function test_withdrawFeeTokens() public {
    ...
    deal(s_sourceFeeToken, address(s_cctpVerifier), feeAmount);

    uint256 initialAggregatorBalance = IERC20(s_sourceFeeToken).balanceOf(FEE_AGGREGATOR);
    uint256 initialVerifierBalance = IERC20(s_sourceFeeToken).balanceOf(address(s_cctpVerifier));
    ...
    vm.prank(STRANGER);
    s_cctpVerifier.withdrawFeeTokens(feeTokens);

    uint256 finalAggregatorBalance = IERC20(s_sourceFeeToken).balanceOf(FEE_AGGREGATOR);
    uint256 finalVerifierBalance = IERC20(s_sourceFeeToken).balanceOf(address(s_cctpVerifier));
  ...
  function test_withdrawFeeTokens_MultipleTokens() public {
    ...
    // Give the verifier some fee tokens.
    deal(s_sourceFeeToken, address(s_cctpVerifier), feeAmount1);
    deal(token2, address(s_cctpVerifier), feeAmount2);
    ...
    assertEq(0, IERC20(s_sourceFeeToken).balanceOf(address(s_cctpVerifier)));
    assertEq(0, IERC20(token2).balanceOf(address(s_cctpVerifier)));

...
contract TokenPool_setRateLimitAdmin is TokenPoolSetup {
  function test_SetRateLimitAdmin() public {
    ...
    vm.expectEmit();
    emit TokenPool.DynamicConfigSet(address(s_sourceRouter), newRateLimitAdmin, s_feeAggregator);
    s_tokenPool.setDynamicConfig(address(s_sourceRouter), newRateLimitAdmin, s_feeAggregator);
  ...
  function test_RevertWhen_SetRateLimitAdmin() public {
    ...
    s_tokenPool.setDynamicConfig(address(s_sourceRouter), STRANGER, STRANGER);
  }
}

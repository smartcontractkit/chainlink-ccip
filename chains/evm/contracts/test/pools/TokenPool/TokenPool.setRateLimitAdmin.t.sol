...
contract TokenPool_setRateLimitAdmin is TokenPoolSetup {
  function test_SetRateLimitAdmin() public {
    ...
    vm.expectEmit();
    emit TokenPool.DynamicConfigSet(address(s_sourceRouter), newRateLimitAdmin, address(0), address(0));
    s_tokenPool.setDynamicConfig(address(s_sourceRouter), newRateLimitAdmin, address(0), address(0));
  ...
  function test_RevertWhen_SetRateLimitAdmin() public {
    ...
    s_tokenPool.setDynamicConfig(address(s_sourceRouter), STRANGER, address(0), address(0));
  }
}

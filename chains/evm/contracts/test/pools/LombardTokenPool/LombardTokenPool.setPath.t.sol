...
contract LombardTokenPool_setPath is LombardTokenPoolSetup {
  ...
  function test_setPath() public {
    ...
    s_pool.setPath(DEST_CHAIN_SELECTOR, L_CHAIN_ID, expectedAllowedCaller, REMOTE_ADAPTER);
  ...
  function test_setPath_RevertWhen_ChainNotSupported() public {
    ...
    s_pool.setPath(999, L_CHAIN_ID, bytes32(uint256(uint160(s_remotePool))), bytes32(0));
  }

  function test_setPath_RevertWhen_ZeroLChainId() public {
    vm.expectRevert(LombardTokenPool.ZeroLombardChainId.selector);
    s_pool.setPath(DEST_CHAIN_SELECTOR, bytes32(0), bytes32(uint256(uint160(s_remotePool))), bytes32(0));
  }
  ...
  function test_setPath_RevertWhen_InvalidRemotePoolForChain() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(
        TokenPool.InvalidRemotePoolForChain.selector, DEST_CHAIN_SELECTOR, abi.encode(invalidAllowedCaller)
      )
    );
    s_pool.setPath(DEST_CHAIN_SELECTOR, L_CHAIN_ID, invalidAllowedCaller, bytes32(0));
  }
}

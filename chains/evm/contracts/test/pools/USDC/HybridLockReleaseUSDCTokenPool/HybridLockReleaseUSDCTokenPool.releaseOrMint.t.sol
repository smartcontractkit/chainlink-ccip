...
contract HybridLockReleaseUSDCTokenPool_releaseOrMint is HybridLockReleaseUSDCTokenPoolSetup {
  function test_OnLockReleaseMechanism() public {
    ...
    USDCTokenPool.SourceTokenDataPayload memory sourceTokenDataPayload = USDCTokenPool.SourceTokenDataPayload({
      ...
      amount: 1e6,
      destinationDomain: SOURCE_DOMAIN_IDENTIFIER,
    ...
    Internal.SourceTokenData memory sourceTokenData = Internal.SourceTokenData({
      ...
      extraData: abi.encode(sourceTokenDataPayload),
    ...
    assertEq(
      s_USDCToken.balanceOf(address(s_lockBox)), liquidityAmount - amount, "Incorrect remaining liquidity in TokenPool"
    );

...
abstract contract TokenPool is IPoolV2, Ownable2StepMsgSender {
  ...
  constructor(
    ...
    if (address(token) == address(0) || router == address(0) || rmnProxy == address(0) || feeAggregator == address(0)) {
  ...
  function setDynamicConfig(address router, address rateLimitAdmin, address feeAggregator) public onlyOwner {
    if (router == address(0) || feeAggregator == address(0)) revert ZeroAddressInvalid();
    s_router = IRouter(router);
  ...
  function withdrawFeeTokens(
    ...
    for (uint256 i = 0; i < feeTokens.length; ++i) {
      ...
      if (feeTokenBalance > 0) {
        ...
        IERC20(feeTokens[i]).safeTransfer(feeAggregator, feeTokenBalance);
        emit FeeTokenWithdrawn(feeAggregator, address(feeTokens[i]), feeTokenBalance);
      }
    }
  }
}

...
contract ERC20LockBox is Ownable2StepMsgSender {
    ...
    mapping(uint64 => uint256) public s_tokenBalances;
    ...
    function deposit(uint256 amount, uint64 remoteChainSelector) external {
        // Note: The caller may be a token pool or a liquidity.
        ...
        s_tokenBalances[remoteChainSelector] += amount;
    ...
    function withdraw(uint256 amount, address recipient, uint64 remoteChainSelector) external {
        ...
        if (s_tokenBalances[remoteChainSelector] < amount) {
            revert InsufficientBalance(remoteChainSelector, amount, s_tokenBalances[remoteChainSelector]);
        }
    ...
    /// @return allowedCallers Array of all addresses that are allowed to call deposit and withdraw.
    function getAllowedCallers() external view returns (address[] memory allowedCallers) {
        return s_allowedCallers.values();
    }

    /// @notice Get the balance for a specific remote chain selector
    /// @param remoteChainSelector The remote chain selector to query
    /// @return balance The balance of tokens for the specified remote chain selector
    function getBalance(uint64 remoteChainSelector) external view returns (uint256 balance) {
        return s_tokenBalances[remoteChainSelector];
    }
}
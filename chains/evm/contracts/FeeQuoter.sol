...
import {IERC165} from "@chainlink/contracts/src/v0.8/vendor/openzeppelin-solidity/v5.0.2/contracts/interfaces/IERC165.sol";
    ...
    KeystoneFeedsPermissionHandler
{
    ...
    error UnsupportedNumberOfTokens(
        ...
        uint256 maxNumberOfTokensPerMsg
    );
    ...
    event PriceFeedPerTokenUpdated(
        address indexed token,
        TokenPriceFeedConfig priceFeedConfig
    );
    ...
    event TokenTransferFeeConfigDeleted(
        uint64 indexed destChainSelector,
        address indexed token
    );
    event PremiumMultiplierWeiPerEthUpdated(
        address indexed token,
    ...
    mapping(address token => Internal.TimestampedPackedUint224 price)
        private s_usdPerToken;

    /// @dev Stores the price data feed configurations per token.
    mapping(address token => TokenPriceFeedConfig dataFeedAddress)
        private s_usdPriceFeedsPerToken;

    /// @dev The multiplier for destination chain specific premiums that can be set by the owner or fee admin.
    mapping(address token => uint64 premiumMultiplierWeiPerEth)
        private s_premiumMultiplierWeiPerEth;

    /// @dev The destination chain specific fee configs.
    mapping(uint64 destChainSelector => DestChainConfig destChainConfig)
        internal s_destChainConfigs;

    /// @dev The token transfer fee config that can be set by the owner or fee admin.
    mapping(uint64 destChainSelector => mapping(address token => TokenTransferFeeConfig tranferFeeConfig))
        private s_tokenTransferFeeConfig;
    ...
    constructor(
        ...
        if (
            ...
            staticConfig.maxFeeJuelsPerMsg == 0 ||
        ...
        i_tokenPriceStalenessThreshold = staticConfig
        ...
        _applyTokenTransferFeeConfigUpdates(
            tokenTransferFeeConfigArgs,
            new TokenTransferFeeConfigRemoveArgs[](0)
        );
    }
    ...
    function getTokenPrice(
        ...
        Internal.TimestampedPackedUint224 memory tokenPrice = s_usdPerToken[
            token
        ];

        // If the token price is not stale, return it.
        if (
            block.timestamp - tokenPrice.timestamp <
        ...
        TokenPriceFeedConfig memory priceFeedConfig = s_usdPriceFeedsPerToken[
            token
        ];
        if (
            !priceFeedConfig.isEnabled ||
            priceFeedConfig.dataFeedAddress == address(0)
        ) {
            return tokenPrice;
        }
        ...
        Internal.TimestampedPackedUint224
            memory feedPrice = _getTokenPriceFromDataFeed(priceFeedConfig);

        // We check if the feed price isn't more stale than the stored price. Return the most recent one.
        return
            feedPrice.timestamp >= tokenPrice.timestamp
                ? feedPrice
                : tokenPrice;
    }
    ...
    function getTokenPrices(
        ...
        Internal.TimestampedPackedUint224[]
            memory tokenPrices = new Internal.TimestampedPackedUint224[](
                length
            );
    ...
    function getTokenAndGasPrices(
        ...
        if (!s_destChainConfigs[destChainSelector].isEnabled)
            revert DestinationChainNotEnabled(destChainSelector);
        return (
            _getValidatedTokenPrice(token),
            _getValidatedGasPrice(
                destChainSelector,
                s_destChainConfigs[destChainSelector].gasPriceStalenessThreshold
            )
        );
    }
    ...
    function convertTokenAmount(
        ...
        return
            (fromTokenAmount * _getValidatedTokenPrice(fromToken)) /
            _getValidatedTokenPrice(toToken);
    }
    ...
    function _getValidatedTokenPrice(
        ...
        Internal.TimestampedPackedUint224 memory tokenPrice = getTokenPrice(
            token
        );
        // Token price must be set at least once.
        if (tokenPrice.timestamp == 0 || tokenPrice.value == 0)
    ...
    {
        AggregatorV3Interface dataFeedContract = AggregatorV3Interface(
            priceFeedConfig.dataFeedAddress
        );
        (
            ,
            // uint80 roundID
        ...
        uint224 rebasedValue = _calculateRebasedValue(
            dataFeedContract.decimals(),
            priceFeedConfig.tokenDecimals,
            uint256(dataFeedAnswer)
        );
    ...
    function _getValidatedGasPrice(
        ...
        Internal.TimestampedPackedUint224
            memory gasPrice = s_usdPerUnitGasByDestChainSelector[
                destChainSelector
            ];
    ...
    function updatePrices(
        ...
        for (uint256 i = 0; i < tokenUpdatesLength; ++i) {
            Internal.TokenPriceUpdate memory update = priceUpdates
                .tokenPriceUpdates[i];
            s_usdPerToken[update.sourceToken] = Internal
        ...
        for (uint256 i = 0; i < gasUpdatesLength; ++i) {
            Internal.GasPriceUpdate memory update = priceUpdates
                .gasPriceUpdates[i];
            s_usdPerUnitGasByDestChainSelector[
                update.destChainSelector
            ] = Internal.TimestampedPackedUint224({
    ...
    function _updateTokenPriceFeeds(
        ...
        for (uint256 i; i < tokenPriceFeedUpdates.length; ++i) {
            ...
            TokenPriceFeedConfig memory tokenPriceFeedConfig = update
                .feedConfig;
    ...
    function onReport(bytes calldata metadata, bytes calldata report) external {
        (
            ...
            bytes2 reportName
        ) = metadata._extractMetadataInfo();

        _validateReportPermission(
            msg.sender,
            workflowOwner,
            workflowName,
            reportName
        ...
        ReceivedCCIPFeedReport[] memory feeds = abi.decode(
            report,
            (ReceivedCCIPFeedReport[])
        );

        for (uint256 i = 0; i < feeds.length; ++i) {
            TokenPriceFeedConfig memory feedConfig = s_usdPriceFeedsPerToken[
                feeds[i].token
            ];
            ...
            uint224 rebasedValue = _calculateRebasedValue(
                uint8(KEYSTONE_PRICE_DECIMALS),
                feedConfig.tokenDecimals,
                feeds[i].price
            );
            ...
            s_usdPerToken[feeds[i].token] = Internal.TimestampedPackedUint224({
                value: rebasedValue,
                timestamp: feeds[i].timestamp
            });
            emit UsdPerTokenUpdated(
                feeds[i].token,
                rebasedValue,
    ...
    function getValidatedFee(
        ...
        DestChainConfig memory destChainConfig = s_destChainConfigs[
            destChainSelector
        ];
        if (!destChainConfig.isEnabled)
            revert DestinationChainNotEnabled(destChainSelector);
        if (!s_feeTokens.contains(message.feeToken))
            revert FeeTokenNotSupported(message.feeToken);
        ...
        uint224 packedGasPrice = _getValidatedGasPrice(
            destChainSelector,
            destChainConfig.gasPriceStalenessThreshold
        );
        ...
        if (numberOfTokens > 0) {
            ...
            premiumFeeUSDWei =
                uint256(destChainConfig.networkFeeUSDCents) *
                1e16;
        }
        ...
        uint256 calldataLength = message.data.length +
            tokenTransferBytesOverhead;
        uint256 destCallDataCost = calldataLength *
            destChainConfig.destGasPerPayloadByteBase;
        if (calldataLength > destChainConfig.destGasPerPayloadByteThreshold) {
            destCallDataCost =
                destChainConfig.destGasPerPayloadByteBase *
                destChainConfig.destGasPerPayloadByteThreshold +
                (calldataLength -
                    destChainConfig.destGasPerPayloadByteThreshold) *
                destChainConfig.destGasPerPayloadByteHigh;
        }
    ...
    function _applyPremiumMultiplierWeiPerEthUpdates(
        ...
        for (uint256 i = 0; i < premiumMultiplierWeiPerEthArgs.length; ++i) {
            ...
            uint64 premiumMultiplierWeiPerEth = premiumMultiplierWeiPerEthArgs[
                i
    ...
    function _getTokenTransferCost(
        ...
        view
        returns (
            uint256 tokenTransferFeeUSDWei,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < numberOfTokens; ++i) {
            ...
            if (!transferFeeConfig.isEnabled) {
                ...
                tokenTransferBytesOverhead += Pool
            ...
            if (transferFeeConfig.deciBps > 0) {
                ...
                bpsFeeUSDWei =
                    (tokenPrice._calcUSDValueFromTokenAmount(
                        tokenAmount.amount
                    ) * transferFeeConfig.deciBps) /
                    1e5;
            }
            ...
            uint256 minFeeUSDWei = uint256(transferFeeConfig.minFeeUSDCents) *
                1e16;
            ...
            uint256 maxFeeUSDWei = uint256(transferFeeConfig.maxFeeUSDCents) *
                1e16;
    ...
    function _calculateRebasedValue(
        ...
        if (excessDecimals > FEE_BASE_DECIMALS) {
            rebasedVal =
                feedValue /
                (10 ** (excessDecimals - FEE_BASE_DECIMALS));
        } else {
            rebasedVal =
                feedValue *
                (10 ** (FEE_BASE_DECIMALS - excessDecimals));
        }
    ...
    function _getDataAvailabilityCost(
        ...
        uint256 dataAvailabilityGas = (dataAvailabilityLengthBytes *
            destChainConfig.destGasPerDataAvailabilityByte) +
        ...
        return
            ((dataAvailabilityGas * dataAvailabilityGasPrice) *
                destChainConfig.destDataAvailabilityMultiplierBps) * 1e14;
    }
    ...
    function _applyTokenTransferFeeConfigUpdates(
        ...
        for (uint256 i = 0; i < tokenTransferFeeConfigArgs.length; ++i) {
            TokenTransferFeeConfigArgs
                memory tokenTransferFeeConfigArg = tokenTransferFeeConfigArgs[
                    i
                ];
            uint64 destChainSelector = tokenTransferFeeConfigArg
                .destChainSelector;

            for (
                uint256 j = 0;
                j < tokenTransferFeeConfigArg.tokenTransferFeeConfigs.length;
                ++j
            ) {
                TokenTransferFeeConfig
                    memory tokenTransferFeeConfig = tokenTransferFeeConfigArg
                        .tokenTransferFeeConfigs[j]
                        .tokenTransferFeeConfig;
                address token = tokenTransferFeeConfigArg
                    .tokenTransferFeeConfigs[j]
                    .token;

                if (
                    tokenTransferFeeConfig.minFeeUSDCents >=
                    tokenTransferFeeConfig.maxFeeUSDCents
                ) {
                ...
                s_tokenTransferFeeConfig[destChainSelector][
                    token
                ] = tokenTransferFeeConfig;
        ...
        for (uint256 i = 0; i < tokensToUseDefaultFeeConfigs.length; ++i) {
            uint64 destChainSelector = tokensToUseDefaultFeeConfigs[i]
                .destChainSelector;
    ...
    function _validateDestFamilyAddress(
        ...
        if (chainFamilySelector == Internal.CHAIN_FAMILY_SELECTOR_SVM) {
            ...
            return
                Internal._validate32ByteAddress(
                    destAddress,
                    gasLimit > 0 ? 1 : 0
                );
        }
        if (
            ...
            return
                Internal._validate32ByteAddress(
                    destAddress,
                    Internal.APTOS_PRECOMPILE_SPACE
                );
        }
    ...
    function _parseGenericExtraArgsFromBytes(
        ...
        Client.GenericExtraArgsV2
            memory parsedExtraArgs = _parseUnvalidatedEVMExtraArgsFromBytes(
                extraArgs,
                defaultTxGasLimit
            );

        if (parsedExtraArgs.gasLimit > maxPerMsgGasLimit)
    ...
    function _parseUnvalidatedEVMExtraArgsFromBytes(
        ...
        if (extraArgsTag == Client.GENERIC_EXTRA_ARGS_V2_TAG) {
            ...
            return
                Client.GenericExtraArgsV2({
                    gasLimit: abi.decode(argsData, (uint256)),
                    allowOutOfOrderExecution: false
                });
        }
        revert InvalidExtraArgsTag();
    ...
    function _validateMessageAndResolveGasLimitForDestination(
        ...
        if (dataLength > uint256(destChainConfig.maxDataBytes)) {
            revert MessageTooLarge(
                uint256(destChainConfig.maxDataBytes),
                dataLength
            );
        }
        if (numberOfTokens > uint256(destChainConfig.maxNumberOfTokensPerMsg)) {
            revert UnsupportedNumberOfTokens(
                numberOfTokens,
                destChainConfig.maxNumberOfTokensPerMsg
            );
        }
        ...
        if (
            ...
            Client.SVMExtraArgsV1
                memory svmExtraArgsV1 = _parseSVMExtraArgsFromBytes(
                    message.extraArgs,
                    destChainConfig.maxPerMsgGasLimit,
                    destChainConfig.enforceOutOfOrder
                );
            ...
            if (abi.decode(message.receiver, (uint256)) == 0) {
                ...
                svmExpandedDataLength += ((accountsLength +
                    Client.SVM_MESSAGING_ACCOUNTS_OVERHEAD) *
                    Client.SVM_ACCOUNT_BYTE_SIZE);
            }

            if (
                numberOfTokens > 0 && svmExtraArgsV1.tokenReceiver == bytes32(0)
            ) {
                revert InvalidTokenReceiver();
            }
            if (accountsLength > Client.SVM_EXTRA_ARGS_MAX_ACCOUNTS) {
                revert TooManySVMExtraArgsAccounts(
                    accountsLength,
                    Client.SVM_EXTRA_ARGS_MAX_ACCOUNTS
                );
            }
            if (svmExtraArgsV1.accountIsWritableBitmap >> accountsLength != 0) {
                revert InvalidSVMExtraArgsWritableBitmap(
                    svmExtraArgsV1.accountIsWritableBitmap,
                    accountsLength
                );
            }

            svmExpandedDataLength += (numberOfTokens *
                Client.SVM_TOKEN_TRANSFER_DATA_OVERHEAD);

            // The token destBytesOverhead can be very different per token so we have to take it into account as well.
            for (uint256 i = 0; i < numberOfTokens; ++i) {
                uint256 destBytesOverhead = s_tokenTransferFeeConfig[
                    destChainSelector
                ][message.tokenAmounts[i].token].destBytesOverhead;
            ...
            revert InvalidChainFamilySelector(
                destChainConfig.chainFamilySelector
            );
        }
        ...
        )
    {
        ...
        if (feeToken == i_linkToken) {
            ...
            msgFeeJuels = convertTokenAmount(
                feeToken,
                feeTokenAmount,
                i_linkToken
            );
        ...
        if (msgFeeJuels > i_maxFeeJuelsPerMsg)
            revert MessageFeeTooHigh(msgFeeJuels, i_maxFeeJuelsPerMsg);

        (
            convertedExtraArgs,
            isOutOfOrderExecution,
            tokenReceiver
        ) = _processChainFamilySelector(
            destChainSelector,
            messageReceiver,
            extraArgs
        );
    ...
    function _processChainFamilySelector(
        ...
        view
        returns (
            bytes memory validatedExtraArgs,
        ...
        )
    {
        ...
        if (
            ...
            return (
                Client._argsToBytes(parsedExtraArgs),
                parsedExtraArgs.allowOutOfOrderExecution,
                messageReceiver
            );
        }
        if (
    ...
    function processPoolReturnData(
        ...
        bytes4 chainFamilySelector = s_destChainConfigs[destChainSelector]
            .chainFamilySelector;
        ...
        for (uint256 i = 0; i < onRampTokenTransfers.length; ++i) {
            ...
            uint256 destPoolDataLength = onRampTokenTransfers[i]
                .extraData
                .length;
            ...
            _validateDestFamilyAddress(
                chainFamilySelector,
                onRampTokenTransfers[i].destTokenAddress,
                1
            );
            FeeQuoter.TokenTransferFeeConfig
                memory tokenTransferFeeConfig = s_tokenTransferFeeConfig[
                    destChainSelector
                ][sourceToken];
    ...
    function _applyDestChainConfigUpdates(
        ...
        for (uint256 i = 0; i < destChainConfigArgs.length; ++i) {
            DestChainConfigArgs memory destChainConfigArg = destChainConfigArgs[
                i
            ];
            uint64 destChainSelector = destChainConfigArgs[i].destChainSelector;
            DestChainConfig memory destChainConfig = destChainConfigArg
                .destChainConfig;

            // destChainSelector must be non-zero, defaultTxGasLimit must be set, must be less than maxPerMsgGasLimit
            if (
                destChainSelector == 0 ||
                destChainConfig.defaultTxGasLimit == 0 ||
                destChainConfig.defaultTxGasLimit >
                destChainConfig.maxPerMsgGasLimit ||
                (destChainConfig.chainFamilySelector !=
                    Internal.CHAIN_FAMILY_SELECTOR_EVM &&
            ...
            if (
                s_destChainConfigs[destChainSelector].chainFamilySelector == 0
            ) {

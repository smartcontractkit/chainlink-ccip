...
library Internal {
    ...
    address public constant GAS_ESTIMATION_SENDER =
    ...
    bytes32 internal constant ANY_2_EVM_MESSAGE_HASH =
        keccak256("Any2EVMMessageHashV1");
    bytes32 internal constant EVM_2_ANY_MESSAGE_HASH =
        keccak256("EVM2AnyMessageHashV1");
    ...
    function _validateEVMAddress(bytes memory encodedAddress) internal pure {
        ...
        if (
            encodedAddressUint > type(uint160).max ||
            encodedAddressUint < EVM_PRECOMPILE_SPACE
        ) {
            revert InvalidEVMAddress(encodedAddress);
        }
    }

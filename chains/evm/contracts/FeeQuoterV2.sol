...
contract FeeQuoterV2 is AuthorizedCallers, IFeeQuoter, ITypeAndVersion, IReceiver, KeystoneFeedsPermissionHandler {
  ...
  string public constant override typeAndVersion = "FeeQuoterV2 1.7.0-dev";
  ...
  function processPoolReturnData(
    ...
    for (uint256 i = 0; i < onRampTokenTransfers.length; ++i) {
      ...
      TokenTransferFeeConfig memory tokenTransferFeeConfig = s_tokenTransferFeeConfig[destChainSelector][sourceToken];
  ...
  function processPoolReturnDataNew(
    ...
    destExecDataPerToken = new bytes[](onRampTokenTransfers.length);
    for (uint256 i = 0; i < onRampTokenTransfers.length; ++i) {
      address sourceToken = onRampTokenTransfers[i].sourceTokenAddress;

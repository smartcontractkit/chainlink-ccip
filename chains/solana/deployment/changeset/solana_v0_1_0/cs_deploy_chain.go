...
func deployChainContractsSolana(
	...
	if chainState.FeeQuoter.IsZero() {
		feeQuoterAddress, err = DeployAndMaybeSaveToAddressBook(e, chain, ab, ds, shared.FeeQuoter, shared.Version1_0_0, false, "")
		if err != nil {
	...
	if chainState.Router.IsZero() {
		// deploy router
		ccipRouterProgram, err = DeployAndMaybeSaveToAddressBook(e, chain, ab, ds, shared.Router, shared.Version1_0_0, false, "")
		if err != nil {
			return batches, fmt.Errorf("failed to deploy program: %w", err)
		}
	...
	if chainState.RMNRemote.IsZero() {
		rmnRemoteAddress, err = DeployAndMaybeSaveToAddressBook(e, chain, ab, ds, shared.RMNRemote, shared.Version1_0_0, false, "")
		if err != nil {
			return batches, fmt.Errorf("failed to deploy program: %w", err)
		}
	...
	if config.UpgradeConfig.NewMCMVersion != nil {
		...
		newTxns, err := generateUpgradeTxns(e, chain, ab, ds, config, config.UpgradeConfig.NewMCMVersion, mcmState.McmProgram, shared.ManyChainMultisigProgram)
		if err != nil {
			return batches, fmt.Errorf("failed to generate upgrade txns: %w", err)
		}
		// create proposals for txns
	...
	if config.UpgradeConfig.NewAccessControllerVersion != nil {
		...
		newTxns, err := generateUpgradeTxns(e, chain, ab, ds, config, config.UpgradeConfig.NewAccessControllerVersion, mcmState.AccessControllerProgram, shared.AccessControllerProgram)
		if err != nil {
			return batches, fmt.Errorf("failed to generate upgrade txns: %w", err)
		}
		// create proposals for txns
	...
	if config.UpgradeConfig.NewTimelockVersion != nil {
		...
		newTxns, err := generateUpgradeTxns(e, chain, ab, ds, config, config.UpgradeConfig.NewTimelockVersion, mcmState.TimelockProgram, shared.RBACTimelockProgram)
		if err != nil {
			return batches, fmt.Errorf("failed to generate upgrade txns: %w", err)
		}
		// create proposals for txns

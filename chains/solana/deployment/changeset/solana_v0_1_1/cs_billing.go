		...
		for dstSelector, dstConfig := range dst {
			...
			for tokenAddress, feeConfig := range dstConfig.TokenAddressToFeeConfig {
				...
				if !curConfig.TokenTransferConfig.IsEnabled {
					...
					minFeeUsdCents := shared.Coalesce(feeConfig.MinFeeUsdcents, minFeeUsdCentsVal)
					maxFeeUsdCents := shared.Coalesce(feeConfig.MaxFeeUsdcents, math.MaxUint32)
					destGasOverhead := shared.Coalesce(feeConfig.DestGasOverhead, uint32(90_000))
					destBytesOverhead := shared.Coalesce(feeConfig.DestBytesOverhead, uint32(32))
					deciBps := shared.Coalesce(feeConfig.DeciBps, uint16(0))
					isEnabled := shared.Coalesce(feeConfig.IsEnabled, true)
				...
				newConfig := solFeeQuoter.TokenTransferFeeConfig{
					MinFeeUsdcents:    shared.Coalesce(feeConfig.MinFeeUsdcents, curConfig.TokenTransferConfig.MinFeeUsdcents),
					MaxFeeUsdcents:    shared.Coalesce(feeConfig.MaxFeeUsdcents, curConfig.TokenTransferConfig.MaxFeeUsdcents),
					DeciBps:           shared.Coalesce(feeConfig.DeciBps, curConfig.TokenTransferConfig.DeciBps),
					DestGasOverhead:   shared.Coalesce(feeConfig.DestGasOverhead, curConfig.TokenTransferConfig.DestGasOverhead),
					DestBytesOverhead: shared.Coalesce(feeConfig.DestBytesOverhead, curConfig.TokenTransferConfig.DestBytesOverhead),
					IsEnabled:         shared.Coalesce(feeConfig.IsEnabled, curConfig.TokenTransferConfig.IsEnabled),
				}

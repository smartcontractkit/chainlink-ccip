		...
		t.Run("Commit", func(t *testing.T) {
			...
			t.Run("When committing a report with a valid source chain selector, merkle root and interval it succeeds", func(t *testing.T) {
				...
				for i, testcase := range priceUpdatesCases {
					t.Run(testcase.Name, func(t *testing.T) {
						_, root := testutils.MakeEvmToSolanaMessage(t, config.CcipTokenReceiver, config.CcipLogicReceiver, config.EvmChainSelector, config.SolanaChainSelector, []byte{1, 2, 3, uint8(i)})
			...
			t.Run("Edge cases", func(t *testing.T) {
				t.Run("When committing a report with an invalid source chain selector it fails", func(t *testing.T) {
					...
					require.NoError(t, err)
					_, root := testutils.MakeEvmToSolanaMessage(t, config.CcipTokenReceiver, config.CcipLogicReceiver, sourceChainSelector, config.SolanaChainSelector, []byte{4, 5, 6})
				...
				t.Run("When committing a report with an invalid interval it fails", func(t *testing.T) {
					t.Parallel()
					_, root := testutils.MakeEvmToSolanaMessage(t, config.CcipTokenReceiver, config.CcipLogicReceiver, config.EvmChainSelector, config.SolanaChainSelector, []byte{4, 5, 6})
					rootPDA, err := ccip.GetCommitReportPDA(config.EvmChainSelector, root)
					require.NoError(t, err)
				...
				t.Run("When committing a report with a repeated merkle root, it fails", func(t *testing.T) {
					t.Parallel()
					_, root := testutils.MakeEvmToSolanaMessage(t, config.CcipTokenReceiver, config.CcipLogicReceiver, config.EvmChainSelector, config.SolanaChainSelector, []byte{1, 2, 3, 1}) // repeated root
					rootPDA, err := ccip.GetCommitReportPDA(config.EvmChainSelector, root)
					require.NoError(t, err)
				...
				t.Run("Invalid price updates", func(t *testing.T) {
					...
					_, root := testutils.MakeEvmToSolanaMessage(t, config.CcipTokenReceiver, config.CcipLogicReceiver, config.EvmChainSelector, config.SolanaChainSelector, []byte{1, 2, 3})
					rootPDA, err := ccip.GetCommitReportPDA(config.EvmChainSelector, root)
					require.NoError(t, err)

...
func ParseAnyAccount(accountData []byte) (any, error) {
	...
	case Account_Config:
		...
		return value, nil
	case Account_NameVersion:
		value := new(NameVersion)
		err := value.UnmarshalWithDecoder(decoder)
		if err != nil {
			return nil, fmt.Errorf("failed to unmarshal account as NameVersion: %w", err)
		}
		return value, nil
	default:
...
func ParseAccount_NameVersion(accountData []byte) (*NameVersion, error) {
	decoder := binary.NewBorshDecoder(accountData)
	discriminator, err := decoder.ReadDiscriminator()
	if err != nil {
		return nil, fmt.Errorf("failed to peek discriminator: %w", err)
	}
	if discriminator != Account_NameVersion {
		return nil, fmt.Errorf("expected discriminator %v, got %s", Account_NameVersion, binary.FormatDiscriminator(discriminator))
	}
	acc := new(NameVersion)
	err = acc.UnmarshalWithDecoder(decoder)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal account of type NameVersion: %w", err)
	}
	return acc, nil
}

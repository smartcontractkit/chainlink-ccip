...
func NewInitGlobalConfigInstruction(
	...
	// Add the accounts to the instruction.
	{
		...
		accounts__.Append(solanago.NewAccountMeta(systemProgramAccount, false, false))
		// Account 3 "program": Read-only, Non-signer, Required, Address: 41FGToCmdaWa1dgZLKFAjvmx6e6AjVTX7SVRibvsMGVB
...
func NewInitializeInstruction(
	...
	// Add the accounts to the instruction.
	{
		...
		// Account 3 "system_program": Read-only, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(systemProgramAccount, false, false))
		// Account 4 "program": Read-only, Non-signer, Required, Address: 41FGToCmdaWa1dgZLKFAjvmx6e6AjVTX7SVRibvsMGVB
		accounts__.Append(solanago.NewAccountMeta(programAccount, false, false))
...
// Builds a "transfer_mint_authority_to_multisig" instruction.
func NewTransferMintAuthorityToMultisigInstruction(
	...
	poolSignerAccount solanago.PublicKey,
	authorityAccount solanago.PublicKey,
	newMultisigMintAuthorityAccount solanago.PublicKey,
	programAccount solanago.PublicKey,
	programDataAccount solanago.PublicKey,
) (solanago.Instruction, error) {
	buf__ := new(bytes.Buffer)
	...
	// Encode the instruction discriminator.
	err := enc__.WriteBytes(Instruction_TransferMintAuthorityToMultisig[:], false)
	if err != nil {
		return nil, fmt.Errorf("failed to write instruction discriminator: %w", err)
	}
	accounts__ := solanago.AccountMetaSlice{}
	...
	// Add the accounts to the instruction.
	{
		...
		accounts__.Append(solanago.NewAccountMeta(authorityAccount, false, true))
		// Account 5 "new_multisig_mint_authority": Read-only, Non-signer, Required
		accounts__.Append(solanago.NewAccountMeta(newMultisigMintAuthorityAccount, false, false))
		// Account 6 "program": Read-only, Non-signer, Required, Address: 41FGToCmdaWa1dgZLKFAjvmx6e6AjVTX7SVRibvsMGVB
		accounts__.Append(solanago.NewAccountMeta(programAccount, false, false))
...
func NewUpdateDefaultRmnInstruction(
	...
	// Add the accounts to the instruction.
	{
		...
		accounts__.Append(solanago.NewAccountMeta(authorityAccount, false, true))
		// Account 2 "program": Read-only, Non-signer, Required, Address: 41FGToCmdaWa1dgZLKFAjvmx6e6AjVTX7SVRibvsMGVB
		accounts__.Append(solanago.NewAccountMeta(programAccount, false, false))
...
type TransferMintAuthorityToMultisigInstruction struct {
	...
	Authority                solanago.PublicKey `json:"authority"`
	AuthoritySigner          bool               `json:"authority_signer"`
	NewMultisigMintAuthority solanago.PublicKey `json:"new_multisig_mint_authority"`
	Program                  solanago.PublicKey `json:"program"`
	ProgramData              solanago.PublicKey `json:"program_data"`
}

func (obj *TransferMintAuthorityToMultisigInstruction) GetDiscriminator() []byte {
	return Instruction_TransferMintAuthorityToMultisig[:]
}

// UnmarshalWithDecoder unmarshals the TransferMintAuthorityToMultisigInstruction from Borsh-encoded bytes prefixed with its discriminator.
func (obj *TransferMintAuthorityToMultisigInstruction) UnmarshalWithDecoder(decoder *binary.Decoder) error {
	// Read the discriminator and check it against the expected value:
	discriminator, err := decoder.ReadDiscriminator()
	if err != nil {
		return fmt.Errorf("failed to read instruction discriminator for %s: %w", "TransferMintAuthorityToMultisigInstruction", err)
	}
	if discriminator != Instruction_TransferMintAuthorityToMultisig {
		return fmt.Errorf("instruction discriminator mismatch for %s: expected %s, got %s", "TransferMintAuthorityToMultisigInstruction", Instruction_TransferMintAuthorityToMultisig, discriminator)
	}
	return nil
}

func (obj *TransferMintAuthorityToMultisigInstruction) UnmarshalAccountIndices(buf []byte) ([]uint8, error) {
	// UnmarshalAccountIndices decodes account indices from Borsh-encoded bytes
	decoder := binary.NewBorshDecoder(buf)
	indices := make([]uint8, 0)
	index := uint8(0)
	...
	indices = append(indices, index)
	// Decode from new_multisig_mint_authority account index
	index = uint8(0)
	err = decoder.Decode(&index)
	if err != nil {
		return nil, fmt.Errorf("failed to decode %s account index: %w", "new_multisig_mint_authority", err)
	}
	indices = append(indices, index)
	// Decode from program account index
	index = uint8(0)
...
func (obj *TransferMintAuthorityToMultisigInstruction) PopulateFromAccountIndices(indices []uint8, accountKeys []solanago.PublicKey) error {
	// PopulateFromAccountIndices sets account public keys from indices and account keys array
	if len(indices) != 8 {
	...
	obj.Authority = accountKeys[indices[indexOffset]]
	indexOffset++
	// Set new_multisig_mint_authority account from index
	if indices[indexOffset] >= uint8(len(accountKeys)) {
		return fmt.Errorf("account index %d for %s is out of bounds (max: %d)", indices[indexOffset], "new_multisig_mint_authority", len(accountKeys)-1)
	}
	obj.NewMultisigMintAuthority = accountKeys[indices[indexOffset]]
	indexOffset++
	// Set program account from index
	if indices[indexOffset] >= uint8(len(accountKeys)) {
		return fmt.Errorf("account index %d for %s is out of bounds (max: %d)", indices[indexOffset], "program", len(accountKeys)-1)
...
func (obj *TransferMintAuthorityToMultisigInstruction) GetAccountKeys() []solanago.PublicKey {
	keys := make([]solanago.PublicKey, 0)
	keys = append(keys, obj.State)
	keys = append(keys, obj.Mint)
	keys = append(keys, obj.TokenProgram)
	keys = append(keys, obj.PoolSigner)
	keys = append(keys, obj.Authority)
	keys = append(keys, obj.NewMultisigMintAuthority)
	keys = append(keys, obj.Program)
	keys = append(keys, obj.ProgramData)
	return keys
}

// Unmarshal unmarshals the TransferMintAuthorityToMultisigInstruction from Borsh-encoded bytes prefixed with the discriminator.
func (obj *TransferMintAuthorityToMultisigInstruction) Unmarshal(buf []byte) error {
	var err error
	err = obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling TransferMintAuthorityToMultisigInstruction: %w", err)
	}
	return nil
}

// UnmarshalTransferMintAuthorityToMultisigInstruction unmarshals the instruction from Borsh-encoded bytes prefixed with the discriminator.
func UnmarshalTransferMintAuthorityToMultisigInstruction(buf []byte) (*TransferMintAuthorityToMultisigInstruction, error) {
	obj := new(TransferMintAuthorityToMultisigInstruction)
	var err error
	err = obj.Unmarshal(buf)
	if err != nil {
		return nil, err
...
func ParseInstruction(instructionData []byte, accountIndicesData []byte, accountKeys []solanago.PublicKey) (Instruction, error) {
	...
	case Instruction_TransferMintAuthorityToMultisig:
		instruction := new(TransferMintAuthorityToMultisigInstruction)
		decoder := binary.NewBorshDecoder(instructionData)
		err := instruction.UnmarshalWithDecoder(decoder)
		if err != nil {
			return nil, fmt.Errorf("failed to unmarshal instruction as TransferMintAuthorityToMultisigInstruction: %w", err)
		}
		if accountIndicesData != nil && len(accountIndicesData) > 0 {
			indices, err := instruction.UnmarshalAccountIndices(accountIndicesData)
			if err != nil {

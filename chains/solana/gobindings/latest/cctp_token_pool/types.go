// Code generated by https://github.com/gagliardetto/anchor-go. DO NOT EDIT.
// This file contains parsers for the types defined in the IDL.

package cctp_token_pool

import (
	"bytes"
	"fmt"
	errors "github.com/gagliardetto/anchor-go/errors"
	binary "github.com/gagliardetto/binary"
	solanago "github.com/gagliardetto/solana-go"
)

type BaseChain struct {
	Remote            RemoteConfig         `json:"remote"`
	InboundRateLimit  RateLimitTokenBucket `json:"inboundRateLimit"`
	OutboundRateLimit RateLimitTokenBucket `json:"outboundRateLimit"`
}

func (obj BaseChain) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Remote`:
	err = encoder.Encode(obj.Remote)
	if err != nil {
		return errors.NewField("Remote", err)
	}
	// Serialize `InboundRateLimit`:
	err = encoder.Encode(obj.InboundRateLimit)
	if err != nil {
		return errors.NewField("InboundRateLimit", err)
	}
	// Serialize `OutboundRateLimit`:
	err = encoder.Encode(obj.OutboundRateLimit)
	if err != nil {
		return errors.NewField("OutboundRateLimit", err)
	}
	return nil
}

func (obj BaseChain) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding BaseChain: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *BaseChain) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Remote`:
	err = decoder.Decode(&obj.Remote)
	if err != nil {
		return errors.NewField("Remote", err)
	}
	// Deserialize `InboundRateLimit`:
	err = decoder.Decode(&obj.InboundRateLimit)
	if err != nil {
		return errors.NewField("InboundRateLimit", err)
	}
	// Deserialize `OutboundRateLimit`:
	err = decoder.Decode(&obj.OutboundRateLimit)
	if err != nil {
		return errors.NewField("OutboundRateLimit", err)
	}
	return nil
}

func (obj *BaseChain) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling BaseChain: %w", err)
	}
	return nil
}

func UnmarshalBaseChain(buf []byte) (*BaseChain, error) {
	obj := new(BaseChain)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type BaseConfig struct {
	TokenProgram          solanago.PublicKey   `json:"tokenProgram"`
	Mint                  solanago.PublicKey   `json:"mint"`
	Decimals              uint8                `json:"decimals"`
	PoolSigner            solanago.PublicKey   `json:"poolSigner"`
	PoolTokenAccount      solanago.PublicKey   `json:"poolTokenAccount"`
	Owner                 solanago.PublicKey   `json:"owner"`
	ProposedOwner         solanago.PublicKey   `json:"proposedOwner"`
	RateLimitAdmin        solanago.PublicKey   `json:"rateLimitAdmin"`
	RouterOnrampAuthority solanago.PublicKey   `json:"routerOnrampAuthority"`
	Router                solanago.PublicKey   `json:"router"`
	Rebalancer            solanago.PublicKey   `json:"rebalancer"`
	CanAcceptLiquidity    bool                 `json:"canAcceptLiquidity"`
	ListEnabled           bool                 `json:"listEnabled"`
	AllowList             []solanago.PublicKey `json:"allowList"`
	RmnRemote             solanago.PublicKey   `json:"rmnRemote"`
}

func (obj BaseConfig) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `TokenProgram`:
	err = encoder.Encode(obj.TokenProgram)
	if err != nil {
		return errors.NewField("TokenProgram", err)
	}
	// Serialize `Mint`:
	err = encoder.Encode(obj.Mint)
	if err != nil {
		return errors.NewField("Mint", err)
	}
	// Serialize `Decimals`:
	err = encoder.Encode(obj.Decimals)
	if err != nil {
		return errors.NewField("Decimals", err)
	}
	// Serialize `PoolSigner`:
	err = encoder.Encode(obj.PoolSigner)
	if err != nil {
		return errors.NewField("PoolSigner", err)
	}
	// Serialize `PoolTokenAccount`:
	err = encoder.Encode(obj.PoolTokenAccount)
	if err != nil {
		return errors.NewField("PoolTokenAccount", err)
	}
	// Serialize `Owner`:
	err = encoder.Encode(obj.Owner)
	if err != nil {
		return errors.NewField("Owner", err)
	}
	// Serialize `ProposedOwner`:
	err = encoder.Encode(obj.ProposedOwner)
	if err != nil {
		return errors.NewField("ProposedOwner", err)
	}
	// Serialize `RateLimitAdmin`:
	err = encoder.Encode(obj.RateLimitAdmin)
	if err != nil {
		return errors.NewField("RateLimitAdmin", err)
	}
	// Serialize `RouterOnrampAuthority`:
	err = encoder.Encode(obj.RouterOnrampAuthority)
	if err != nil {
		return errors.NewField("RouterOnrampAuthority", err)
	}
	// Serialize `Router`:
	err = encoder.Encode(obj.Router)
	if err != nil {
		return errors.NewField("Router", err)
	}
	// Serialize `Rebalancer`:
	err = encoder.Encode(obj.Rebalancer)
	if err != nil {
		return errors.NewField("Rebalancer", err)
	}
	// Serialize `CanAcceptLiquidity`:
	err = encoder.Encode(obj.CanAcceptLiquidity)
	if err != nil {
		return errors.NewField("CanAcceptLiquidity", err)
	}
	// Serialize `ListEnabled`:
	err = encoder.Encode(obj.ListEnabled)
	if err != nil {
		return errors.NewField("ListEnabled", err)
	}
	// Serialize `AllowList`:
	err = encoder.Encode(obj.AllowList)
	if err != nil {
		return errors.NewField("AllowList", err)
	}
	// Serialize `RmnRemote`:
	err = encoder.Encode(obj.RmnRemote)
	if err != nil {
		return errors.NewField("RmnRemote", err)
	}
	return nil
}

func (obj BaseConfig) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding BaseConfig: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *BaseConfig) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `TokenProgram`:
	err = decoder.Decode(&obj.TokenProgram)
	if err != nil {
		return errors.NewField("TokenProgram", err)
	}
	// Deserialize `Mint`:
	err = decoder.Decode(&obj.Mint)
	if err != nil {
		return errors.NewField("Mint", err)
	}
	// Deserialize `Decimals`:
	err = decoder.Decode(&obj.Decimals)
	if err != nil {
		return errors.NewField("Decimals", err)
	}
	// Deserialize `PoolSigner`:
	err = decoder.Decode(&obj.PoolSigner)
	if err != nil {
		return errors.NewField("PoolSigner", err)
	}
	// Deserialize `PoolTokenAccount`:
	err = decoder.Decode(&obj.PoolTokenAccount)
	if err != nil {
		return errors.NewField("PoolTokenAccount", err)
	}
	// Deserialize `Owner`:
	err = decoder.Decode(&obj.Owner)
	if err != nil {
		return errors.NewField("Owner", err)
	}
	// Deserialize `ProposedOwner`:
	err = decoder.Decode(&obj.ProposedOwner)
	if err != nil {
		return errors.NewField("ProposedOwner", err)
	}
	// Deserialize `RateLimitAdmin`:
	err = decoder.Decode(&obj.RateLimitAdmin)
	if err != nil {
		return errors.NewField("RateLimitAdmin", err)
	}
	// Deserialize `RouterOnrampAuthority`:
	err = decoder.Decode(&obj.RouterOnrampAuthority)
	if err != nil {
		return errors.NewField("RouterOnrampAuthority", err)
	}
	// Deserialize `Router`:
	err = decoder.Decode(&obj.Router)
	if err != nil {
		return errors.NewField("Router", err)
	}
	// Deserialize `Rebalancer`:
	err = decoder.Decode(&obj.Rebalancer)
	if err != nil {
		return errors.NewField("Rebalancer", err)
	}
	// Deserialize `CanAcceptLiquidity`:
	err = decoder.Decode(&obj.CanAcceptLiquidity)
	if err != nil {
		return errors.NewField("CanAcceptLiquidity", err)
	}
	// Deserialize `ListEnabled`:
	err = decoder.Decode(&obj.ListEnabled)
	if err != nil {
		return errors.NewField("ListEnabled", err)
	}
	// Deserialize `AllowList`:
	err = decoder.Decode(&obj.AllowList)
	if err != nil {
		return errors.NewField("AllowList", err)
	}
	// Deserialize `RmnRemote`:
	err = decoder.Decode(&obj.RmnRemote)
	if err != nil {
		return errors.NewField("RmnRemote", err)
	}
	return nil
}

func (obj *BaseConfig) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling BaseConfig: %w", err)
	}
	return nil
}

func UnmarshalBaseConfig(buf []byte) (*BaseConfig, error) {
	obj := new(BaseConfig)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type CcipAccountMeta struct {
	Pubkey     solanago.PublicKey `json:"pubkey"`
	IsSigner   bool               `json:"isSigner"`
	IsWritable bool               `json:"isWritable"`
}

func (obj CcipAccountMeta) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Pubkey`:
	err = encoder.Encode(obj.Pubkey)
	if err != nil {
		return errors.NewField("Pubkey", err)
	}
	// Serialize `IsSigner`:
	err = encoder.Encode(obj.IsSigner)
	if err != nil {
		return errors.NewField("IsSigner", err)
	}
	// Serialize `IsWritable`:
	err = encoder.Encode(obj.IsWritable)
	if err != nil {
		return errors.NewField("IsWritable", err)
	}
	return nil
}

func (obj CcipAccountMeta) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding CcipAccountMeta: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *CcipAccountMeta) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Pubkey`:
	err = decoder.Decode(&obj.Pubkey)
	if err != nil {
		return errors.NewField("Pubkey", err)
	}
	// Deserialize `IsSigner`:
	err = decoder.Decode(&obj.IsSigner)
	if err != nil {
		return errors.NewField("IsSigner", err)
	}
	// Deserialize `IsWritable`:
	err = decoder.Decode(&obj.IsWritable)
	if err != nil {
		return errors.NewField("IsWritable", err)
	}
	return nil
}

func (obj *CcipAccountMeta) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling CcipAccountMeta: %w", err)
	}
	return nil
}

func UnmarshalCcipAccountMeta(buf []byte) (*CcipAccountMeta, error) {
	obj := new(CcipAccountMeta)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type CcipCctpFundingConfigChanged struct {
	Old FundingConfig `json:"old"`
	New FundingConfig `json:"new"`
}

func (obj CcipCctpFundingConfigChanged) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Old`:
	err = encoder.Encode(obj.Old)
	if err != nil {
		return errors.NewField("Old", err)
	}
	// Serialize `New`:
	err = encoder.Encode(obj.New)
	if err != nil {
		return errors.NewField("New", err)
	}
	return nil
}

func (obj CcipCctpFundingConfigChanged) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding CcipCctpFundingConfigChanged: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *CcipCctpFundingConfigChanged) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Old`:
	err = decoder.Decode(&obj.Old)
	if err != nil {
		return errors.NewField("Old", err)
	}
	// Deserialize `New`:
	err = decoder.Decode(&obj.New)
	if err != nil {
		return errors.NewField("New", err)
	}
	return nil
}

func (obj *CcipCctpFundingConfigChanged) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling CcipCctpFundingConfigChanged: %w", err)
	}
	return nil
}

func UnmarshalCcipCctpFundingConfigChanged(buf []byte) (*CcipCctpFundingConfigChanged, error) {
	obj := new(CcipCctpFundingConfigChanged)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type CcipCctpMessageEventAccountClosed struct {
	OriginalSender      solanago.PublicKey `json:"originalSender"`
	RemoteChainSelector uint64             `json:"remoteChainSelector"`
	MsgTotalNonce       uint64             `json:"msgTotalNonce"`
	Address             solanago.PublicKey `json:"address"`
}

func (obj CcipCctpMessageEventAccountClosed) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `OriginalSender`:
	err = encoder.Encode(obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Serialize `RemoteChainSelector`:
	err = encoder.Encode(obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Serialize `MsgTotalNonce`:
	err = encoder.Encode(obj.MsgTotalNonce)
	if err != nil {
		return errors.NewField("MsgTotalNonce", err)
	}
	// Serialize `Address`:
	err = encoder.Encode(obj.Address)
	if err != nil {
		return errors.NewField("Address", err)
	}
	return nil
}

func (obj CcipCctpMessageEventAccountClosed) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding CcipCctpMessageEventAccountClosed: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *CcipCctpMessageEventAccountClosed) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `OriginalSender`:
	err = decoder.Decode(&obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Deserialize `RemoteChainSelector`:
	err = decoder.Decode(&obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Deserialize `MsgTotalNonce`:
	err = decoder.Decode(&obj.MsgTotalNonce)
	if err != nil {
		return errors.NewField("MsgTotalNonce", err)
	}
	// Deserialize `Address`:
	err = decoder.Decode(&obj.Address)
	if err != nil {
		return errors.NewField("Address", err)
	}
	return nil
}

func (obj *CcipCctpMessageEventAccountClosed) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling CcipCctpMessageEventAccountClosed: %w", err)
	}
	return nil
}

func UnmarshalCcipCctpMessageEventAccountClosed(buf []byte) (*CcipCctpMessageEventAccountClosed, error) {
	obj := new(CcipCctpMessageEventAccountClosed)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type CcipCctpMessageSentEvent struct {
	OriginalSender      solanago.PublicKey `json:"originalSender"`
	RemoteChainSelector uint64             `json:"remoteChainSelector"`
	MsgTotalNonce       uint64             `json:"msgTotalNonce"`
	EventAddress        solanago.PublicKey `json:"eventAddress"`
	SourceDomain        uint32             `json:"sourceDomain"`
	CctpNonce           uint64             `json:"cctpNonce"`
	MessageSentBytes    []byte             `json:"messageSentBytes"`
}

func (obj CcipCctpMessageSentEvent) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `OriginalSender`:
	err = encoder.Encode(obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Serialize `RemoteChainSelector`:
	err = encoder.Encode(obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Serialize `MsgTotalNonce`:
	err = encoder.Encode(obj.MsgTotalNonce)
	if err != nil {
		return errors.NewField("MsgTotalNonce", err)
	}
	// Serialize `EventAddress`:
	err = encoder.Encode(obj.EventAddress)
	if err != nil {
		return errors.NewField("EventAddress", err)
	}
	// Serialize `SourceDomain`:
	err = encoder.Encode(obj.SourceDomain)
	if err != nil {
		return errors.NewField("SourceDomain", err)
	}
	// Serialize `CctpNonce`:
	err = encoder.Encode(obj.CctpNonce)
	if err != nil {
		return errors.NewField("CctpNonce", err)
	}
	// Serialize `MessageSentBytes`:
	err = encoder.Encode(obj.MessageSentBytes)
	if err != nil {
		return errors.NewField("MessageSentBytes", err)
	}
	return nil
}

func (obj CcipCctpMessageSentEvent) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding CcipCctpMessageSentEvent: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *CcipCctpMessageSentEvent) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `OriginalSender`:
	err = decoder.Decode(&obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Deserialize `RemoteChainSelector`:
	err = decoder.Decode(&obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Deserialize `MsgTotalNonce`:
	err = decoder.Decode(&obj.MsgTotalNonce)
	if err != nil {
		return errors.NewField("MsgTotalNonce", err)
	}
	// Deserialize `EventAddress`:
	err = decoder.Decode(&obj.EventAddress)
	if err != nil {
		return errors.NewField("EventAddress", err)
	}
	// Deserialize `SourceDomain`:
	err = decoder.Decode(&obj.SourceDomain)
	if err != nil {
		return errors.NewField("SourceDomain", err)
	}
	// Deserialize `CctpNonce`:
	err = decoder.Decode(&obj.CctpNonce)
	if err != nil {
		return errors.NewField("CctpNonce", err)
	}
	// Deserialize `MessageSentBytes`:
	err = decoder.Decode(&obj.MessageSentBytes)
	if err != nil {
		return errors.NewField("MessageSentBytes", err)
	}
	return nil
}

func (obj *CcipCctpMessageSentEvent) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling CcipCctpMessageSentEvent: %w", err)
	}
	return nil
}

func UnmarshalCcipCctpMessageSentEvent(buf []byte) (*CcipCctpMessageSentEvent, error) {
	obj := new(CcipCctpMessageSentEvent)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type CctpChain struct {
	DomainId          uint32             `json:"domainId"`
	DestinationCaller solanago.PublicKey `json:"destinationCaller"`
}

func (obj CctpChain) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `DomainId`:
	err = encoder.Encode(obj.DomainId)
	if err != nil {
		return errors.NewField("DomainId", err)
	}
	// Serialize `DestinationCaller`:
	err = encoder.Encode(obj.DestinationCaller)
	if err != nil {
		return errors.NewField("DestinationCaller", err)
	}
	return nil
}

func (obj CctpChain) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding CctpChain: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *CctpChain) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `DomainId`:
	err = decoder.Decode(&obj.DomainId)
	if err != nil {
		return errors.NewField("DomainId", err)
	}
	// Deserialize `DestinationCaller`:
	err = decoder.Decode(&obj.DestinationCaller)
	if err != nil {
		return errors.NewField("DestinationCaller", err)
	}
	return nil
}

func (obj *CctpChain) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling CctpChain: %w", err)
	}
	return nil
}

func UnmarshalCctpChain(buf []byte) (*CctpChain, error) {
	obj := new(CctpChain)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type ChainConfig struct {
	Version uint8     `json:"version"`
	Base    BaseChain `json:"base"`
	Cctp    CctpChain `json:"cctp"`
}

func (obj ChainConfig) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Version`:
	err = encoder.Encode(obj.Version)
	if err != nil {
		return errors.NewField("Version", err)
	}
	// Serialize `Base`:
	err = encoder.Encode(obj.Base)
	if err != nil {
		return errors.NewField("Base", err)
	}
	// Serialize `Cctp`:
	err = encoder.Encode(obj.Cctp)
	if err != nil {
		return errors.NewField("Cctp", err)
	}
	return nil
}

func (obj ChainConfig) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding ChainConfig: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *ChainConfig) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Version`:
	err = decoder.Decode(&obj.Version)
	if err != nil {
		return errors.NewField("Version", err)
	}
	// Deserialize `Base`:
	err = decoder.Decode(&obj.Base)
	if err != nil {
		return errors.NewField("Base", err)
	}
	// Deserialize `Cctp`:
	err = decoder.Decode(&obj.Cctp)
	if err != nil {
		return errors.NewField("Cctp", err)
	}
	return nil
}

func (obj *ChainConfig) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling ChainConfig: %w", err)
	}
	return nil
}

func UnmarshalChainConfig(buf []byte) (*ChainConfig, error) {
	obj := new(ChainConfig)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type DeriveAccountsResponse struct {
	AskAgainWith       []CcipAccountMeta    `json:"askAgainWith"`
	AccountsToSave     []CcipAccountMeta    `json:"accountsToSave"`
	LookUpTablesToSave []solanago.PublicKey `json:"lookUpTablesToSave"`
	CurrentStage       string               `json:"currentStage"`
	NextStage          string               `json:"nextStage"`
}

func (obj DeriveAccountsResponse) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `AskAgainWith`:
	err = encoder.Encode(obj.AskAgainWith)
	if err != nil {
		return errors.NewField("AskAgainWith", err)
	}
	// Serialize `AccountsToSave`:
	err = encoder.Encode(obj.AccountsToSave)
	if err != nil {
		return errors.NewField("AccountsToSave", err)
	}
	// Serialize `LookUpTablesToSave`:
	err = encoder.Encode(obj.LookUpTablesToSave)
	if err != nil {
		return errors.NewField("LookUpTablesToSave", err)
	}
	// Serialize `CurrentStage`:
	err = encoder.Encode(obj.CurrentStage)
	if err != nil {
		return errors.NewField("CurrentStage", err)
	}
	// Serialize `NextStage`:
	err = encoder.Encode(obj.NextStage)
	if err != nil {
		return errors.NewField("NextStage", err)
	}
	return nil
}

func (obj DeriveAccountsResponse) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding DeriveAccountsResponse: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *DeriveAccountsResponse) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `AskAgainWith`:
	err = decoder.Decode(&obj.AskAgainWith)
	if err != nil {
		return errors.NewField("AskAgainWith", err)
	}
	// Deserialize `AccountsToSave`:
	err = decoder.Decode(&obj.AccountsToSave)
	if err != nil {
		return errors.NewField("AccountsToSave", err)
	}
	// Deserialize `LookUpTablesToSave`:
	err = decoder.Decode(&obj.LookUpTablesToSave)
	if err != nil {
		return errors.NewField("LookUpTablesToSave", err)
	}
	// Deserialize `CurrentStage`:
	err = decoder.Decode(&obj.CurrentStage)
	if err != nil {
		return errors.NewField("CurrentStage", err)
	}
	// Deserialize `NextStage`:
	err = decoder.Decode(&obj.NextStage)
	if err != nil {
		return errors.NewField("NextStage", err)
	}
	return nil
}

func (obj *DeriveAccountsResponse) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling DeriveAccountsResponse: %w", err)
	}
	return nil
}

func UnmarshalDeriveAccountsResponse(buf []byte) (*DeriveAccountsResponse, error) {
	obj := new(DeriveAccountsResponse)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type FundingConfig struct {
	Manager            solanago.PublicKey `json:"manager"`
	ReclaimDestination solanago.PublicKey `json:"reclaimDestination"`
	MinimumSignerFunds uint64             `json:"minimumSignerFunds"`
}

func (obj FundingConfig) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Manager`:
	err = encoder.Encode(obj.Manager)
	if err != nil {
		return errors.NewField("Manager", err)
	}
	// Serialize `ReclaimDestination`:
	err = encoder.Encode(obj.ReclaimDestination)
	if err != nil {
		return errors.NewField("ReclaimDestination", err)
	}
	// Serialize `MinimumSignerFunds`:
	err = encoder.Encode(obj.MinimumSignerFunds)
	if err != nil {
		return errors.NewField("MinimumSignerFunds", err)
	}
	return nil
}

func (obj FundingConfig) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding FundingConfig: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *FundingConfig) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Manager`:
	err = decoder.Decode(&obj.Manager)
	if err != nil {
		return errors.NewField("Manager", err)
	}
	// Deserialize `ReclaimDestination`:
	err = decoder.Decode(&obj.ReclaimDestination)
	if err != nil {
		return errors.NewField("ReclaimDestination", err)
	}
	// Deserialize `MinimumSignerFunds`:
	err = decoder.Decode(&obj.MinimumSignerFunds)
	if err != nil {
		return errors.NewField("MinimumSignerFunds", err)
	}
	return nil
}

func (obj *FundingConfig) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling FundingConfig: %w", err)
	}
	return nil
}

func UnmarshalFundingConfig(buf []byte) (*FundingConfig, error) {
	obj := new(FundingConfig)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type LockOrBurnInV1 struct {
	Receiver            []byte             `json:"receiver"`
	RemoteChainSelector uint64             `json:"remoteChainSelector"`
	OriginalSender      solanago.PublicKey `json:"originalSender"`
	Amount              uint64             `json:"amount"`
	LocalToken          solanago.PublicKey `json:"localToken"`
	MsgTotalNonce       uint64             `json:"msgTotalNonce"`
}

func (obj LockOrBurnInV1) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Receiver`:
	err = encoder.Encode(obj.Receiver)
	if err != nil {
		return errors.NewField("Receiver", err)
	}
	// Serialize `RemoteChainSelector`:
	err = encoder.Encode(obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Serialize `OriginalSender`:
	err = encoder.Encode(obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Serialize `Amount`:
	err = encoder.Encode(obj.Amount)
	if err != nil {
		return errors.NewField("Amount", err)
	}
	// Serialize `LocalToken`:
	err = encoder.Encode(obj.LocalToken)
	if err != nil {
		return errors.NewField("LocalToken", err)
	}
	// Serialize `MsgTotalNonce`:
	err = encoder.Encode(obj.MsgTotalNonce)
	if err != nil {
		return errors.NewField("MsgTotalNonce", err)
	}
	return nil
}

func (obj LockOrBurnInV1) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding LockOrBurnInV1: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *LockOrBurnInV1) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Receiver`:
	err = decoder.Decode(&obj.Receiver)
	if err != nil {
		return errors.NewField("Receiver", err)
	}
	// Deserialize `RemoteChainSelector`:
	err = decoder.Decode(&obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Deserialize `OriginalSender`:
	err = decoder.Decode(&obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Deserialize `Amount`:
	err = decoder.Decode(&obj.Amount)
	if err != nil {
		return errors.NewField("Amount", err)
	}
	// Deserialize `LocalToken`:
	err = decoder.Decode(&obj.LocalToken)
	if err != nil {
		return errors.NewField("LocalToken", err)
	}
	// Deserialize `MsgTotalNonce`:
	err = decoder.Decode(&obj.MsgTotalNonce)
	if err != nil {
		return errors.NewField("MsgTotalNonce", err)
	}
	return nil
}

func (obj *LockOrBurnInV1) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling LockOrBurnInV1: %w", err)
	}
	return nil
}

func UnmarshalLockOrBurnInV1(buf []byte) (*LockOrBurnInV1, error) {
	obj := new(LockOrBurnInV1)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type LockOrBurnOutV1 struct {
	DestTokenAddress RemoteAddress `json:"destTokenAddress"`
	DestPoolData     []byte        `json:"destPoolData"`
}

func (obj LockOrBurnOutV1) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `DestTokenAddress`:
	err = encoder.Encode(obj.DestTokenAddress)
	if err != nil {
		return errors.NewField("DestTokenAddress", err)
	}
	// Serialize `DestPoolData`:
	err = encoder.Encode(obj.DestPoolData)
	if err != nil {
		return errors.NewField("DestPoolData", err)
	}
	return nil
}

func (obj LockOrBurnOutV1) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding LockOrBurnOutV1: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *LockOrBurnOutV1) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `DestTokenAddress`:
	err = decoder.Decode(&obj.DestTokenAddress)
	if err != nil {
		return errors.NewField("DestTokenAddress", err)
	}
	// Deserialize `DestPoolData`:
	err = decoder.Decode(&obj.DestPoolData)
	if err != nil {
		return errors.NewField("DestPoolData", err)
	}
	return nil
}

func (obj *LockOrBurnOutV1) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling LockOrBurnOutV1: %w", err)
	}
	return nil
}

func UnmarshalLockOrBurnOutV1(buf []byte) (*LockOrBurnOutV1, error) {
	obj := new(LockOrBurnOutV1)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type PoolConfig struct {
	Version uint8 `json:"version"`
}

func (obj PoolConfig) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Version`:
	err = encoder.Encode(obj.Version)
	if err != nil {
		return errors.NewField("Version", err)
	}
	return nil
}

func (obj PoolConfig) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding PoolConfig: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *PoolConfig) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Version`:
	err = decoder.Decode(&obj.Version)
	if err != nil {
		return errors.NewField("Version", err)
	}
	return nil
}

func (obj *PoolConfig) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling PoolConfig: %w", err)
	}
	return nil
}

func UnmarshalPoolConfig(buf []byte) (*PoolConfig, error) {
	obj := new(PoolConfig)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type RateLimitConfig struct {
	Enabled  bool   `json:"enabled"`
	Capacity uint64 `json:"capacity"`
	Rate     uint64 `json:"rate"`
}

func (obj RateLimitConfig) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Enabled`:
	err = encoder.Encode(obj.Enabled)
	if err != nil {
		return errors.NewField("Enabled", err)
	}
	// Serialize `Capacity`:
	err = encoder.Encode(obj.Capacity)
	if err != nil {
		return errors.NewField("Capacity", err)
	}
	// Serialize `Rate`:
	err = encoder.Encode(obj.Rate)
	if err != nil {
		return errors.NewField("Rate", err)
	}
	return nil
}

func (obj RateLimitConfig) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding RateLimitConfig: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *RateLimitConfig) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Enabled`:
	err = decoder.Decode(&obj.Enabled)
	if err != nil {
		return errors.NewField("Enabled", err)
	}
	// Deserialize `Capacity`:
	err = decoder.Decode(&obj.Capacity)
	if err != nil {
		return errors.NewField("Capacity", err)
	}
	// Deserialize `Rate`:
	err = decoder.Decode(&obj.Rate)
	if err != nil {
		return errors.NewField("Rate", err)
	}
	return nil
}

func (obj *RateLimitConfig) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling RateLimitConfig: %w", err)
	}
	return nil
}

func UnmarshalRateLimitConfig(buf []byte) (*RateLimitConfig, error) {
	obj := new(RateLimitConfig)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type RateLimitTokenBucket struct {
	Tokens      uint64          `json:"tokens"`
	LastUpdated uint64          `json:"lastUpdated"`
	Cfg         RateLimitConfig `json:"cfg"`
}

func (obj RateLimitTokenBucket) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Tokens`:
	err = encoder.Encode(obj.Tokens)
	if err != nil {
		return errors.NewField("Tokens", err)
	}
	// Serialize `LastUpdated`:
	err = encoder.Encode(obj.LastUpdated)
	if err != nil {
		return errors.NewField("LastUpdated", err)
	}
	// Serialize `Cfg`:
	err = encoder.Encode(obj.Cfg)
	if err != nil {
		return errors.NewField("Cfg", err)
	}
	return nil
}

func (obj RateLimitTokenBucket) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding RateLimitTokenBucket: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *RateLimitTokenBucket) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Tokens`:
	err = decoder.Decode(&obj.Tokens)
	if err != nil {
		return errors.NewField("Tokens", err)
	}
	// Deserialize `LastUpdated`:
	err = decoder.Decode(&obj.LastUpdated)
	if err != nil {
		return errors.NewField("LastUpdated", err)
	}
	// Deserialize `Cfg`:
	err = decoder.Decode(&obj.Cfg)
	if err != nil {
		return errors.NewField("Cfg", err)
	}
	return nil
}

func (obj *RateLimitTokenBucket) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling RateLimitTokenBucket: %w", err)
	}
	return nil
}

func UnmarshalRateLimitTokenBucket(buf []byte) (*RateLimitTokenBucket, error) {
	obj := new(RateLimitTokenBucket)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type ReleaseOrMintInV1 struct {
	OriginalSender      RemoteAddress      `json:"originalSender"`
	RemoteChainSelector uint64             `json:"remoteChainSelector"`
	Receiver            solanago.PublicKey `json:"receiver"`
	Amount              [32]uint8          `json:"amount"`
	LocalToken          solanago.PublicKey `json:"localToken"`

	// @dev WARNING: sourcePoolAddress should be checked prior to any processing of funds. Make sure it matches the
	// expected pool address for the given remoteChainSelector.
	SourcePoolAddress RemoteAddress `json:"sourcePoolAddress"`
	SourcePoolData    []byte        `json:"sourcePoolData"`

	// @dev WARNING: offchainTokenData is untrusted data.
	OffchainTokenData []byte `json:"offchainTokenData"`
}

func (obj ReleaseOrMintInV1) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `OriginalSender`:
	err = encoder.Encode(obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Serialize `RemoteChainSelector`:
	err = encoder.Encode(obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Serialize `Receiver`:
	err = encoder.Encode(obj.Receiver)
	if err != nil {
		return errors.NewField("Receiver", err)
	}
	// Serialize `Amount`:
	err = encoder.Encode(obj.Amount)
	if err != nil {
		return errors.NewField("Amount", err)
	}
	// Serialize `LocalToken`:
	err = encoder.Encode(obj.LocalToken)
	if err != nil {
		return errors.NewField("LocalToken", err)
	}
	// Serialize `SourcePoolAddress`:
	err = encoder.Encode(obj.SourcePoolAddress)
	if err != nil {
		return errors.NewField("SourcePoolAddress", err)
	}
	// Serialize `SourcePoolData`:
	err = encoder.Encode(obj.SourcePoolData)
	if err != nil {
		return errors.NewField("SourcePoolData", err)
	}
	// Serialize `OffchainTokenData`:
	err = encoder.Encode(obj.OffchainTokenData)
	if err != nil {
		return errors.NewField("OffchainTokenData", err)
	}
	return nil
}

func (obj ReleaseOrMintInV1) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding ReleaseOrMintInV1: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *ReleaseOrMintInV1) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `OriginalSender`:
	err = decoder.Decode(&obj.OriginalSender)
	if err != nil {
		return errors.NewField("OriginalSender", err)
	}
	// Deserialize `RemoteChainSelector`:
	err = decoder.Decode(&obj.RemoteChainSelector)
	if err != nil {
		return errors.NewField("RemoteChainSelector", err)
	}
	// Deserialize `Receiver`:
	err = decoder.Decode(&obj.Receiver)
	if err != nil {
		return errors.NewField("Receiver", err)
	}
	// Deserialize `Amount`:
	err = decoder.Decode(&obj.Amount)
	if err != nil {
		return errors.NewField("Amount", err)
	}
	// Deserialize `LocalToken`:
	err = decoder.Decode(&obj.LocalToken)
	if err != nil {
		return errors.NewField("LocalToken", err)
	}
	// Deserialize `SourcePoolAddress`:
	err = decoder.Decode(&obj.SourcePoolAddress)
	if err != nil {
		return errors.NewField("SourcePoolAddress", err)
	}
	// Deserialize `SourcePoolData`:
	err = decoder.Decode(&obj.SourcePoolData)
	if err != nil {
		return errors.NewField("SourcePoolData", err)
	}
	// Deserialize `OffchainTokenData`:
	err = decoder.Decode(&obj.OffchainTokenData)
	if err != nil {
		return errors.NewField("OffchainTokenData", err)
	}
	return nil
}

func (obj *ReleaseOrMintInV1) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling ReleaseOrMintInV1: %w", err)
	}
	return nil
}

func UnmarshalReleaseOrMintInV1(buf []byte) (*ReleaseOrMintInV1, error) {
	obj := new(ReleaseOrMintInV1)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type ReleaseOrMintOutV1 struct {
	DestinationAmount uint64 `json:"destinationAmount"`
}

func (obj ReleaseOrMintOutV1) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `DestinationAmount`:
	err = encoder.Encode(obj.DestinationAmount)
	if err != nil {
		return errors.NewField("DestinationAmount", err)
	}
	return nil
}

func (obj ReleaseOrMintOutV1) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding ReleaseOrMintOutV1: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *ReleaseOrMintOutV1) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `DestinationAmount`:
	err = decoder.Decode(&obj.DestinationAmount)
	if err != nil {
		return errors.NewField("DestinationAmount", err)
	}
	return nil
}

func (obj *ReleaseOrMintOutV1) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling ReleaseOrMintOutV1: %w", err)
	}
	return nil
}

func UnmarshalReleaseOrMintOutV1(buf []byte) (*ReleaseOrMintOutV1, error) {
	obj := new(ReleaseOrMintOutV1)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type RemoteAddress struct {
	Address []byte `json:"address"`
}

func (obj RemoteAddress) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Address`:
	err = encoder.Encode(obj.Address)
	if err != nil {
		return errors.NewField("Address", err)
	}
	return nil
}

func (obj RemoteAddress) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding RemoteAddress: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *RemoteAddress) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Address`:
	err = decoder.Decode(&obj.Address)
	if err != nil {
		return errors.NewField("Address", err)
	}
	return nil
}

func (obj *RemoteAddress) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling RemoteAddress: %w", err)
	}
	return nil
}

func UnmarshalRemoteAddress(buf []byte) (*RemoteAddress, error) {
	obj := new(RemoteAddress)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type RemoteChainCctpConfigChanged struct {
	Config CctpChain `json:"config"`
}

func (obj RemoteChainCctpConfigChanged) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Config`:
	err = encoder.Encode(obj.Config)
	if err != nil {
		return errors.NewField("Config", err)
	}
	return nil
}

func (obj RemoteChainCctpConfigChanged) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding RemoteChainCctpConfigChanged: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *RemoteChainCctpConfigChanged) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Config`:
	err = decoder.Decode(&obj.Config)
	if err != nil {
		return errors.NewField("Config", err)
	}
	return nil
}

func (obj *RemoteChainCctpConfigChanged) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling RemoteChainCctpConfigChanged: %w", err)
	}
	return nil
}

func UnmarshalRemoteChainCctpConfigChanged(buf []byte) (*RemoteChainCctpConfigChanged, error) {
	obj := new(RemoteChainCctpConfigChanged)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type RemoteConfig struct {
	PoolAddresses []RemoteAddress `json:"poolAddresses"`
	TokenAddress  RemoteAddress   `json:"tokenAddress"`
	Decimals      uint8           `json:"decimals"`
}

func (obj RemoteConfig) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `PoolAddresses`:
	err = encoder.Encode(obj.PoolAddresses)
	if err != nil {
		return errors.NewField("PoolAddresses", err)
	}
	// Serialize `TokenAddress`:
	err = encoder.Encode(obj.TokenAddress)
	if err != nil {
		return errors.NewField("TokenAddress", err)
	}
	// Serialize `Decimals`:
	err = encoder.Encode(obj.Decimals)
	if err != nil {
		return errors.NewField("Decimals", err)
	}
	return nil
}

func (obj RemoteConfig) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding RemoteConfig: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *RemoteConfig) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `PoolAddresses`:
	err = decoder.Decode(&obj.PoolAddresses)
	if err != nil {
		return errors.NewField("PoolAddresses", err)
	}
	// Deserialize `TokenAddress`:
	err = decoder.Decode(&obj.TokenAddress)
	if err != nil {
		return errors.NewField("TokenAddress", err)
	}
	// Deserialize `Decimals`:
	err = decoder.Decode(&obj.Decimals)
	if err != nil {
		return errors.NewField("Decimals", err)
	}
	return nil
}

func (obj *RemoteConfig) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling RemoteConfig: %w", err)
	}
	return nil
}

func UnmarshalRemoteConfig(buf []byte) (*RemoteConfig, error) {
	obj := new(RemoteConfig)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

type State struct {
	Version uint8         `json:"version"`
	Config  BaseConfig    `json:"config"`
	Funding FundingConfig `json:"funding"`
}

func (obj State) MarshalWithEncoder(encoder *binary.Encoder) (err error) {
	// Serialize `Version`:
	err = encoder.Encode(obj.Version)
	if err != nil {
		return errors.NewField("Version", err)
	}
	// Serialize `Config`:
	err = encoder.Encode(obj.Config)
	if err != nil {
		return errors.NewField("Config", err)
	}
	// Serialize `Funding`:
	err = encoder.Encode(obj.Funding)
	if err != nil {
		return errors.NewField("Funding", err)
	}
	return nil
}

func (obj State) Marshal() ([]byte, error) {
	buf := bytes.NewBuffer(nil)
	encoder := binary.NewBorshEncoder(buf)
	err := obj.MarshalWithEncoder(encoder)
	if err != nil {
		return nil, fmt.Errorf("error while encoding State: %w", err)
	}
	return buf.Bytes(), nil
}

func (obj *State) UnmarshalWithDecoder(decoder *binary.Decoder) (err error) {
	// Deserialize `Version`:
	err = decoder.Decode(&obj.Version)
	if err != nil {
		return errors.NewField("Version", err)
	}
	// Deserialize `Config`:
	err = decoder.Decode(&obj.Config)
	if err != nil {
		return errors.NewField("Config", err)
	}
	// Deserialize `Funding`:
	err = decoder.Decode(&obj.Funding)
	if err != nil {
		return errors.NewField("Funding", err)
	}
	return nil
}

func (obj *State) Unmarshal(buf []byte) error {
	err := obj.UnmarshalWithDecoder(binary.NewBorshDecoder(buf))
	if err != nil {
		return fmt.Errorf("error while unmarshaling State: %w", err)
	}
	return nil
}

func UnmarshalState(buf []byte) (*State, error) {
	obj := new(State)
	err := obj.Unmarshal(buf)
	if err != nil {
		return nil, err
	}
	return obj, nil
}

// Code generated by https://github.com/gagliardetto/anchor-go. DO NOT EDIT.

package example_ccip_receiver

import (
	"encoding/json"
	"errors"
	"fmt"
	ag_jsonrpc "github.com/gagliardetto/solana-go/rpc/jsonrpc"
)

var (
	_ *json.Encoder        = nil
	_ *ag_jsonrpc.RPCError = nil
	_ fmt.Formatter        = nil
	_                      = errors.ErrUnsupported
)
var (
	ErrOnlyRouter = &customErrorDef{
		code: 6000,
		msg:  "Address is not router external execution PDA",
		name: "OnlyRouter",
	}
	ErrInvalidRouter = &customErrorDef{
		code: 6001,
		msg:  "Invalid router address",
		name: "InvalidRouter",
	}
	ErrInvalidChainAndSender = &customErrorDef{
		code: 6002,
		msg:  "Invalid combination of chain and sender",
		name: "InvalidChainAndSender",
	}
	ErrOnlyOwner = &customErrorDef{
		code: 6003,
		msg:  "Address is not owner",
		name: "OnlyOwner",
	}
	ErrOnlyProposedOwner = &customErrorDef{
		code: 6004,
		msg:  "Address is not proposed_owner",
		name: "OnlyProposedOwner",
	}
	ErrInvalidCaller = &customErrorDef{
		code: 6005,
		msg:  "Caller is not allowed",
		name: "InvalidCaller",
	}
	ErrInvalidProposedOwner = &customErrorDef{
		code: 6006,
		msg:  "Proposed owner is invalid",
		name: "InvalidProposedOwner",
	}
	Errors = map[int]CustomError{
		6000: ErrOnlyRouter,
		6001: ErrInvalidRouter,
		6002: ErrInvalidChainAndSender,
		6003: ErrOnlyOwner,
		6004: ErrOnlyProposedOwner,
		6005: ErrInvalidCaller,
		6006: ErrInvalidProposedOwner,
	}
)

type CustomError interface {
	Code() int
	Name() string
	Error() string
}

type customErrorDef struct {
	code int
	name string
	msg  string
}

func (e *customErrorDef) Code() int {
	return e.code
}

func (e *customErrorDef) Name() string {
	return e.name
}

func (e *customErrorDef) Error() string {
	return fmt.Sprintf("%s(%d): %s", e.name, e.code, e.msg)
}

func DecodeCustomError(rpcErr error) (err error, ok bool) {
	if errCode, o := decodeErrorCode(rpcErr); o {
		if customErr, o := Errors[errCode]; o {
			err = customErr
			ok = true
			return
		}
	}
	return
}

func decodeErrorCode(rpcErr error) (errorCode int, ok bool) {
	var jErr *ag_jsonrpc.RPCError
	if errors.As(rpcErr, &jErr) && jErr.Data != nil {
		if root, o := jErr.Data.(map[string]interface{}); o {
			if rootErr, o := root["err"].(map[string]interface{}); o {
				if rootErrInstructionError, o := rootErr["InstructionError"]; o {
					if rootErrInstructionErrorItems, o := rootErrInstructionError.([]interface{}); o {
						if len(rootErrInstructionErrorItems) == 2 {
							if v, o := rootErrInstructionErrorItems[1].(map[string]interface{}); o {
								if v2, o := v["Custom"].(json.Number); o {
									if code, err := v2.Int64(); err == nil {
										ok = true
										errorCode = int(code)
									}
								} else if v2, o := v["Custom"].(float64); o {
									ok = true
									errorCode = int(v2)
								}
							}
						}
					}
				}
			}
		}
	}
	return
}

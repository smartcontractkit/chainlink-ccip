...
// GetAllConfigsForChain retrieves all the configs for a given chain. It is a flexible
...
func (l *DefaultAccessor) GetAllConfigsForChain(
	ctx context.Context,
	...
	if l.chainSelector == destChainSelector {
		lggr.Debugw("getting configs for destination chain", "chainSelector", l.chainSelector)
		configRequests, standardOffRampRequestCount = prepareDestChainRequest(sourceChainSelectors)
	} else {
		lggr.Debugw("getting configs for source chain", "chainSelector", l.chainSelector)
		configRequests = prepareSourceChainRequest()
	}
...
func prepareSourceChainRequest() contractreader.ExtendedBatchGetLatestValuesRequest {
	var (

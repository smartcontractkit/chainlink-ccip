...
var (
	...
	// getPremiumMultiplierWeiPerEth(address) returns (uint256)
	selectorFQGetPremiumMultiplierWeiPerEth = common.HexToSelector("061877e3")
...
func ViewFeeQuoter(ctx *views.ViewContext) (map[string]any, error) {
	...
	// Get fee tokens with metadata and premium multiplier
...
// getFeeQuoterERC20Decimals fetches decimals from an ERC20 token at tokenAddr.
...
// getFeeQuoterPremiumMultiplierWeiPerEth calls FeeQuoter.getPremiumMultiplierWeiPerEth(feeToken) and returns the uint256 as string.
func getFeeQuoterPremiumMultiplierWeiPerEth(ctx *views.ViewContext, tokenAddrHex string) (string, error) {
	...
	data, err := common.ExecuteCall(ctx, selectorFQGetPremiumMultiplierWeiPerEth, common.EncodeAddress(tokenAddr))
	if err != nil {
		return "", err
	}
	if len(data) < 32 {
...
// getFeeQuoterFeeTokensEnriched returns fee tokens with name, symbol, decimals, and premiumMultiplierWeiPerEth per token.

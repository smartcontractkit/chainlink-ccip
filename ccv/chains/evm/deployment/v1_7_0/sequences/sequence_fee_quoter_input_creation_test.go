...
import (
	...
	fqops_v1_6_0 "github.com/smartcontractkit/chainlink-ccip/chains/evm/deployment/v1_6_0/operations/fee_quoter"
		...
		Metadata: seq1_6.FeeQuoterImportConfigSequenceOutput{
			RemoteChainCfgs: map[uint64]seq1_6.FeeQuoterImportConfigSequenceOutputPerRemoteChain{
				15971525489660198786: {
					DestChainCfg: fqops_v1_6_0.DestChainConfig{
						IsEnabled:                         true,
					...
					TokenTransferFeeCfgs: map[common.Address]fqops_v1_6_0.TokenTransferFeeConfig{
			...
			StaticCfg: fqops_v1_6_0.StaticConfig{
...
func getExpectedOutput() map[uint64]sequence1_7.FeeQuoterUpdate {
	...
	// Chain 5009297550715157269: Has FeeQuoter v1.6.3 (code looks for 1.6.0) + OnRamp v1.5.0.
	// V163 returns empty so only V150 contributes; dest config comes from OnRamp's single RemoteChainSelector (4949039107694359620).
	// PriceUpdaters from V150: CommitStores on this chain only (0x222...2221).
	...
	// Chain 4949039107694359620: Has FeeQuoter v1.6.3 (code looks for 1.6.0) + OnRamp v1.5.0.
	// V163 returns empty so only V150 contributes; OnRamp has single RemoteChainSelector (5009297550715157269).
	// PriceUpdaters are CommitStores on this chain only (0x99 and 0x33).
	...
	// Chain 5936861837188149645: Has FeeQuoter v1.6.3 (version mismatch: code looks for 1.6.0) + OnRamp v1.5.0.
	// V163 returns empty so V150 supplies ConstructorArgs; only OnRamp's RemoteChainSelector (15971525489660198786) is used.
	// MaxFeeJuelsPerMsg from OnRamp MaxNopFeesJuels (9e17). PriceUpdaters: CommitStores on this chain only (0x555...51).
	expected[5936861837188149645] = sequence1_7.FeeQuoterUpdate{
		ChainSelector: 5936861837188149645,
		ConstructorArgs: fqops.ConstructorArgs{
			StaticConfig: fqops.StaticConfig{
				LinkToken:         linkToken,
				MaxFeeJuelsPerMsg: big.NewInt(900000000000000000), // 9e17 from OnRamp MaxNopFeesJuels
			},
			PriceUpdaters: []common.Address{

...
import (
	...
	fqseq "github.com/smartcontractkit/chainlink-ccip/ccv/chains/evm/deployment/v2_0_0/sequences"
)
	...
	return cldf_ops.NewSequence(
		...
		func(b cldf_ops.Bundle, chains cldf_chain.BlockChains, input deploy.FeeQuoterUpdateInput) (output FeeQUpdateArgs, err error) {
			...
			if empty, err := out.IsEmpty(); err != nil || empty {
				return zero, fmt.Errorf("could not create input for fee quoter 2.0 update sequence: %w", err)
			}

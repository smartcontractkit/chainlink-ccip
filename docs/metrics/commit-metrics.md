...
| `ccip_commit_discovery_state{destChainID,discovered}` | ✅ Implemented | Per-round 0/1 readiness gauge: has the plugin discovered its contracts yet (`1` = yes)? A plugin still in discovery can't produce observations, so `0` here is the *reason* other metrics are missing/limited — proxy for "a single unhealthy RPC left the DON stuck in discovery" that heartbeats alone can't surface. Set from `contractsInitialized.Load()` in `commit/plugin.go`'s `Observation()` and reported by `TrackDiscoveryState`. |
...
- **No heartbeat / liveness signal.** `commit/plugin.go:434` (`Outcome()`). If the whole round pipeline wedges (deadlock, a panic-recovery masking a crash loop, OCR itself failing to schedule rounds), every metric in this doc simply stops updating instead of showing a bad value. A stale gauge and a valid-but-bad gauge look identical unless you specifically alert on absence/staleness — easy to forget when every other alert here is threshold-based. This should be step 0 of any triage tree, and today it isn't answerable from commit-plugin metrics at all. → **Implemented**: counter `ccip_commit_plugin_heartbeat{chainFamily, chainID, phase="observation"|"outcome"}`, incremented unconditionally at the very top of `Observation()`/`Outcome()`, before any decode, state check, or early return.
  ...
  **A complement to heartbeat — contract discovery state.** Heartbeats answer "is it scheduling rounds," not "is it *ready* to produce observations." A plugin stuck in its contract-discovery phase keeps ticking heartbeats but emits nothing else useful — the exact shape seen when a single unhealthy RPC leaves the DON stuck in discovery (per-round, only a `config_digest_check_error` rejection surfaced, with no other metrics to run the health/uncommitted runbooks off of). → **Implemented**: gauge `ccip_commit_discovery_state{destChainID,discovered}` (value `1` = out of discovery, `0` = still discovering), read from `contractsInitialized.Load()` in `commit/plugin.go`'s `Observation()` and recorded via `TrackDiscoveryState` (`commit/metrics/prom.go`). Because it's recorded every round even while discovering, `0` is a real "not ready" signal and is what makes a perpetually-in-discovery plugin diagnosable from metrics at all. This is why the health runbook gates on it before heartbeats: a `0` here is the *reason* observation/outcome metrics are missing or limited.

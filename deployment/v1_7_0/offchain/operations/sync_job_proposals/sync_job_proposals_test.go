...
func TestSyncJobProposals_StatusChange_UpdatesDatastore(t *testing.T) {
	...
	require.NoError(t, offchain.SaveJob(ds, localJob))
...
func TestSyncJobProposals_JobNotFoundInJD_MarkedAsOrphanedAndDeleted(t *testing.T) {
	...
	_, err = offchain.GetJob(report.Output.DataStore.Seal(), "nop-1", "nop-1-default-executor")
	require.Error(t, err)
}
...
func TestSyncJobProposals_NOPFilter_OnlySyncsFilteredNOPs(t *testing.T) {
	...
	require.NoError(t, offchain.SaveJob(ds, job1))
	require.NoError(t, offchain.SaveJob(ds, job2))
	...
	job2After, err := offchain.GetJob(report.Output.DataStore.Seal(), "nop-2", "nop-2-default-executor")
	require.NoError(t, err)
	assert.Equal(t, shared.JobProposalStatusPending, job2After.LatestStatus())
	assert.Empty(t, job2After.ActiveProposalID)
}

func TestSyncJobProposals_ListProposalsError_RecordsErrorAndContinues(t *testing.T) {
	ds := datastore.NewMemoryDataStore()
	...
	assert.Equal(t, shared.NOPAlias("nop-1"), report.Output.Errors[0].NOPAlias)
...
func TestMapJDStatusToLocal_AllStatusesMapped(t *testing.T) {
	tests := []struct {
		...
		{"unspecified_defaults_to_pending", jobv1.ProposalStatus_PROPOSAL_STATUS_UNSPECIFIED, shared.JobProposalStatusPending},
		{"unknown_value_defaults_to_pending", jobv1.ProposalStatus(999), shared.JobProposalStatusPending},
	}

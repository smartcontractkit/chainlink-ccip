...
// for a specific chain family (v2.0.0 version).
type GasUpdateAdapter interface {
	...
	// mapping table defines (OnRamp.DestChainConfig.BaseExecutionGasCost, FeeQuoter fields,
	// verifier fields, token pool fields) for one lane, keyed by field name matching the
	// FieldSpec.Name values in fields.go.
	...
	// contract this lane's writes touch (may be multiple for v2.0.0, which touches OnRamp,
	// FeeQuoter, CommitteeVerifier, etc.). resolved is keyed the same way as ReadDestGasFields's return.
	...
	// GasForCallExactCheck, OffRamp MaxGasBufferToUpdateState) purely to flag an unexpected deployment.
	// Returns field name -> value; caller compares against the expected baseline and only logs, never writes.

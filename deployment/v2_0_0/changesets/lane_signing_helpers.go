...
// signerAddressForNOPAlias resolves a NOP's signer address for the given chain family.
// It checks the static topology first, then falls back to JD-fetched keys. With JD
// now pushing both variants (EVM address + raw pubkey) at registration, no cross-family
// derivation is needed — both variants are available directly from JD.

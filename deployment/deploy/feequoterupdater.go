...
func updateFeeQuoterApply(fquRegistry *FQAndRampUpdaterRegistry, mcmsRegistry *changesets.MCMSReaderRegistry) func(cldf.Environment, UpdateFeeQuoterInput) (cldf.ChangesetOutput, error) {
	return func(e cldf.Environment, input UpdateFeeQuoterInput) (cldf.ChangesetOutput, error) {
		...
		for chainSel, perChainInput := range input.Chains {
			...
			if !ok {
				return cldf.ChangesetOutput{}, fmt.Errorf("no FeeQuoterUpdater adapter registered for chain selector %d and version %s", chainSel, perChainInput.FeeQuoterVersion)
			}
			rampUpdater, ok := fquRegistry.GetRampUpdater(chainSel, perChainInput.RampsVersion)
			if !ok {
				return cldf.ChangesetOutput{}, fmt.Errorf("no RampUpdater adapter registered for chain selector %d and version %s", chainSel, perChainInput.RampsVersion)
			}
			versionResolver, ok := fquRegistry.GetConfigImporterVersionResolver(chainSel)
			if !ok {
				return cldf.ChangesetOutput{}, fmt.Errorf("no ConfigImporterVersionResolver adapter registered for chain selector %d", chainSel)
			}
			...
			for _, version := range configImporterVersions {
				...
				if !ok {
					return cldf.ChangesetOutput{}, fmt.Errorf("no ConfigImporter adapter registered for chain selector %d and version %s", chainSel, version)
				}

...
## CCTPv2
Circle's CCTPv2 (Cross Chain Transfer Protocol v2) allows CCIP to transfer USDC cross-chain without fees or the use of
...
`Message.Header.TxHash`. There are two ways to get `sourceDomainId`: we can either define a static map from 
...
// Note that not every RampTokenAmount in Message.TokenAmounts is a CCTPv2 USDC transfer

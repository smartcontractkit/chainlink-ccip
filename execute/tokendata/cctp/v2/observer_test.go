...
func TestDecodeSourceTokenDataPayloadV2(t *testing.T) {
	t.Run("Valid decode with CCTP_VERSION_2_TAG", func(t *testing.T) {
		...
		// Decode
		result, _ := DecodeSourceTokenDataPayloadV2(cciptypes.Bytes(payload))
...
func Test_convertToTokenDataObservations(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			result := observer.convertToTokenDataObservations(
				context.Background(),
				logger.Test(t),
				tc.messages,
				tc.attestations,

...
func Test_buildAttestationStatuses(t *testing.T) {
	...
	t.Run("Error - attestation status not complete", func(t *testing.T) {
		// Create CCTP message with pending status
		cctpMsg := CCTPv2Message{
			Message:     "0xaabbccdd",
			Attestation: "0x11223344",
		...
		status := result[1][msgTokenID]
		require.Error(t, status.Error)
		assert.Contains(t, status.Error.Error(), "attestation status is not complete: pending")
	})
}

...
func Test_NewHTTPClient_New(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.api, func(t *testing.T) {
			client, err := newHTTPClient(logger.Test(t), USDC, tc.api, 1*time.Millisecond, longTimeout, maxCoolDownDuration)
			if tc.wantErr {
				require.Error(t, err)
			} else {
...
func Test_HTTPClient_Get(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			client, err := newHTTPClient(logger.Test(t), USDC, attestationURI.String(), tc.timeout, tc.timeout, maxCoolDownDuration)
			require.NoError(t, err)
...
func Test_HTTPClient_Cooldown(t *testing.T) {
	...
	client, err := newHTTPClient(logger.Test(t), USDC, attestationURI.String(), 1*time.Millisecond, longTimeout, maxCoolDownDuration)
	require.NoError(t, err)
...
func Test_HTTPClient_GetInstance(t *testing.T) {
	...
	client1, err := GetHTTPClient(logger.Test(t), USDC, ts.URL, 1*time.Hour, longTimeout, maxCoolDownDuration)
	require.NoError(t, err)

	client2, err := GetHTTPClient(logger.Test(t), USDC, ts.URL, 1*time.Hour, longTimeout, maxCoolDownDuration)
	require.NoError(t, err)

	client3, err := newHTTPClient(logger.Test(t), USDC, ts.URL, 1*time.Hour, longTimeout, maxCoolDownDuration)
	require.NoError(t, err)
...
func Test_HTTPClient_CoolDownWithRetryHeader(t *testing.T) {
	...
	client, err := newHTTPClient(logger.Test(t), USDC, attestationURI.String(), 1*time.Millisecond, time.Hour, maxCoolDownDuration)
	require.NoError(t, err)
	_, _, err = client.Get(tests.Context(t), [32]byte{1, 2, 3})
	require.EqualError(t, err, tokendata.ErrUnknownResponse.Error())
...
func Test_HTTPClient_RateLimiting_Parallel(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			client, err := newHTTPClient(lggr, USDC, attestationURI.String(), tc.rateConfig, longTimeout, maxCoolDownDuration)
			require.NoError(t, err)

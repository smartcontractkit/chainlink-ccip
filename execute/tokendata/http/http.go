...
type HTTPClient interface {
	...
	Post(ctx context.Context, requestData cciptypes.Bytes) (cciptypes.Bytes, HTTPStatus, error)
}
...
func GetHTTPClient(
	...
	client, err := newHTTPClient(lggr, tokenSymbol, api, apiInterval, apiTimeout, coolDownDuration)
	if err != nil {
		return nil, err
	}
...
func newHTTPClient(
	...
	return newObservedHTTPClient(client, lggr, tokenSymbol), nil
}
...
func (h *httpClient) Get(ctx context.Context, messageHash cciptypes.Bytes32) (cciptypes.Bytes, HTTPStatus, error) {
	...
	requestURL := *h.apiURL
	requestURL.Path = path.Join(requestURL.Path, messageHash.String())

	response, httpStatus, err := h.callAPI(ctx, http.MethodGet, requestURL, nil)
	h.lggr.Debugw(
...
func (h *httpClient) Post(ctx context.Context, requestData cciptypes.Bytes) (cciptypes.Bytes, HTTPStatus, error) {
	response, httpStatus, err := h.callAPI(ctx, http.MethodPost, *h.apiURL, bytes.NewBuffer(requestData))
	h.lggr.Debugw(
		"Response from attestation API",
		"status", httpStatus,
		"err", err,

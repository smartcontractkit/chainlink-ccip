...
func (s *HTTPClient) Get(ctx context.Context, url string, timeout time.Duration) ([]byte, int, http.Header, error) {
	// Use a timeout to guard against attestation API hanging, causing observation timeout and failing
	//to make any progress.
	timeoutCtx, cancel := context.WithTimeoutCause(ctx, timeout, ErrTimeout)
	defer cancel()
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			return nil, http.StatusRequestTimeout, nil, ErrTimeout
		}
	...
	if res.StatusCode == http.StatusTooManyRequests {
		return nil, res.StatusCode, res.Header, ErrRateLimit
	}

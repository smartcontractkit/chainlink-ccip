...
func calculateMessageMaxGas(gasLimit uint64, numRequests, dataLen, numTokens int) (uint64, error) {
	gasOverHeadGas := maxGasOverHeadGas(numRequests, dataLen, numTokens)
	messageMaxGas := gasLimit + gasOverHeadGas

	if messageMaxGas < gasLimit || messageMaxGas < gasOverHeadGas {
		return 0, fmt.Errorf("message max gas overflow, gasLimit=%d gasOverHeadGas=%d", gasLimit, gasOverHeadGas)
	}

...
func (p *Processor) Query(ctx context.Context, prevOutcome Outcome) (Query, error) {
	...
	if err != nil {
		p.metricsReporter.TrackRmnReport(time.Since(rmnReportStart).Seconds(), uint64(p.destChain), false)
	...
	p.metricsReporter.TrackRmnReport(time.Since(rmnReportStart).Seconds(), uint64(p.destChain), true)

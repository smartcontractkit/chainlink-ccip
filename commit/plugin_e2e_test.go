...
func prepareCcipReaderMock(
	...
	ccipReader.EXPECT().GetOffRampSourceChainsConfig(mock.Anything).
		Return(map[ccipocr3.ChainSelector]reader2.SourceChainConfig{
			sourceChain1: {IsEnabled: true, IsRMNVerificationDisabled: false},
			sourceChain2: {IsEnabled: true, IsRMNVerificationDisabled: false},
		}, nil).Maybe()

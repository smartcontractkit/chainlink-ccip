...
// Threshold is the minimum number of observations required for a given chain.
type Threshold uint

// MultiThreshold is an interface that maps a chain selector to a threshold.
type MultiThreshold[T any] interface {
	// Get the minimum number of observations required for a given chain.
	Get(chain T) (Threshold, bool)
}
...
// MakeMultiThreshold constructs a threshold that maps each chain to a threshold value.
func MakeMultiThreshold[K comparable](fChain map[K]int, mapping func(int) Threshold) MultiThreshold[K] {
	...
	for chain, f := range fChain {
		thresh[chain] = mapping(f)
	}
	return thresh
}
...
func (mt multiThreshold[K]) Get(chain K) (Threshold, bool) {
	thresh, ok := mt[chain]
	return thresh, ok
}

// MakeConstantThreshold constructs a threshold that maps each chain to a constant threshold value.
